...
fun StandApp() {
    ...
    var missingSizeFrames by remember { mutableStateOf(0) }
    var displayMode by remember { mutableStateOf(UsbDisplayMode.PROD) }
    var sendUsbCommand by remember { mutableStateOf<((String) -> Boolean)?>(null) }
    ...
    MaterialTheme {
        UsbSerialBinder(
            enabled = true,
            onCommandSenderChanged = { sender -> sendUsbCommand = sender },
            onRawLine = { line ->
            ...
            onJson = { obj ->
                ...
                val rawHasFullSize =
                    raw.widthMm != null && raw.depthMm != null && raw.heightMm != null

                missingSizeFrames = if (rawHasFullSize) {
                    0
                } else {
                    missingSizeFrames + 1
                }
                ...
                val forceWeightOnly =
                    !rawHasFullSize && missingSizeFrames >= MISSING_SIZE_FRAMES_BEFORE_WEIGHT_ONLY

                val merged = prev.copy(
                    widthMm = when {
                        raw.widthMm != null -> mergeWithTolerance(prev.widthMm, raw.widthMm, SIZE_TOLERANCE_MM)
                        forceWeightOnly -> null
                        else -> prev.widthMm
                    },
                    depthMm = when {
                        raw.depthMm != null -> mergeWithTolerance(prev.depthMm, raw.depthMm, SIZE_TOLERANCE_MM)
                        forceWeightOnly -> null
                        else -> prev.depthMm
                    },
                    heightMm = when {
                        raw.heightMm != null -> mergeWithTolerance(prev.heightMm, raw.heightMm, SIZE_TOLERANCE_MM)
                        forceWeightOnly -> null
                        else -> prev.heightMm
                    },
        ...
        Scaffold(
            topBar = {
                StandTopBar(
                    ...
                    onCalibrate = {
                        val ok = sendUsbCommand?.invoke("C") == true
                        scope.launch {
                            snackbarHostState.showSnackbar(
                                if (ok) "Команда CALIB отправлена" else "USB не подключен"
                            )
                        }
                    },
...
fun UsbSerialBinder(
    enabled: Boolean,
    onCommandSenderChanged: (((String) -> Boolean)?) -> Unit,
    ...
    LaunchedEffect(port) {
        if (port == null) {
            onCommandSenderChanged(null)
        } else {
            onCommandSenderChanged { cmd ->
                val bytes = cmd.toByteArray(Charset.forName("UTF-8"))
                val activePort = port ?: return@onCommandSenderChanged false
                scope.launch(Dispatchers.IO) {
                    try {
                        activePort.write(bytes, 200)
                        withContext(Dispatchers.Main) {
                            onStatus("Команда отправлена: $cmd")
                        }
                    } catch (e: Exception) {
                        withContext(Dispatchers.Main) {
                            onStatus("Ошибка отправки команды: ${e.message}")
                        }
                    }
                }
                true
...
private const val MISSING_SIZE_FRAMES_BEFORE_WEIGHT_ONLY = 8
...
fun MeasurementValues.hasDimensions(): Boolean =
    widthMm != null && depthMm != null && heightMm != null

fun MeasurementValues.mode(): String =
    if (hasDimensions()) "full" else "weight_only"
...
fun buildMeasurementValues(calibrated: UsbCalibratedMetrics): MeasurementValues? {
    ...
    return if (hasFullSize) {
        MeasurementValues(w, d, h, weight)
    } else {
        MeasurementValues(
            widthMm = null,
            depthMm = null,
...
fun isWithinTolerance(previous: MeasurementValues, current: MeasurementValues, cfg: DeviceConfig): Boolean {
    ...
    if (weightDelta > cfg.weightTolG) return false

    val previousHasSize = previous.hasDimensions()
    val currentHasSize = current.hasDimensions()

    if (previousHasSize != currentHasSize) return false
    ...
    return widthDelta <= cfg.dimTolMm &&
...
private fun measurementKey(values: MeasurementValues): String {
    ...
    if (!values.hasDimensions()) {
        return "weight_only:$weightQ"
    }
        ...
        val json = JSONObject().apply {
            ...
            if (values != null) {
                put("measurements", JSONObject().apply {
                    put("measurement_mode", values.mode())
                    put("weight_g", values.weightGrams)
                    put("width_mm", values.widthMm ?: JSONObject.NULL)
                    put("depth_mm", values.depthMm ?: JSONObject.NULL)
                    put("height_mm", values.heightMm ?: JSONObject.NULL)
                })
            } else {
