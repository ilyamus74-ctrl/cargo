...
//print_r($_POST);
    if($_POST['toDo'] == "delete") {$sqlUpd="UPDATE `zs_announce_auto` SET `active_announce`='D' WHERE `id` = '".$dbcnx->real_escape_string($_POST['id'])."'"; }
    if($_POST['toDo'] == "published") {
	if(!empty($_POST['newName'])) { $addon=" , `name_announce` = '".$dbcnx->real_escape_string($_POST['newName'])."'";}
