...
if(!empty($i_save_sudmit) and !empty($i_view_wp) and !empty($i_preg_table)) 
	{
	$sql_check="SELECT `id`,`id_wp` FROM `$i_preg_table` WHERE `id_wp`='$i_view_wp'";
	...
	list($id_ch,$id_wp_ch)=mysql_fetch_row($sql_ch);
		if(empty($id_wp_ch)) 
				...
				//$name_=2;
				$sql_i="INSERT INTO $i_preg_table (`id_wp`,`name`,`main_url`,`url`,`date_in`,`preg_one`,`preg_link`,`preg_name`,`preg_anonce`,`preg_img`,`preg_text`,`preg_f_img`,`active`,`rss`) VALUES ('$i_view_wp','$name_','$i_main_link','$i_url_link',now(),'$i_preg_one','$i_preg_link','$i_preg_name','$i_preg_anonce','$i_preg_img','$i_preg_text','$i_preg_f_img','$i_active','$i_rss')";
				$in_new_wp=mysql_query("$sql_i") or die("Invalid query: " . mysql_error());
				//echo "insert $i_preg_table $sql_i";
				}
			    else {
				//echo"update";
				$sql_upd=mysql_query("UPDATE $i_preg_table SET `main_url`='$i_main_link',`url`='$i_url_link',`preg_one`='$i_preg_one',`preg_link`='$i_preg_link',`preg_name`='$i_preg_name',`preg_anonce`='$i_preg_anonce',`preg_img`='$i_preg_img',`preg_text`='$i_preg_text',`preg_f_img`='$i_preg_f_img',`active`='$i_active',`rss`='$i_rss' WHERE `id_wp`='$i_view_wp' ") or die ("Invalid query: " . mysql_error());
				}
	$view_wp=$i_view_wp;
	}
//delete 
...
while ($row = mysql_fetch_row($result)) {
	...
	{
		$qquurr="SELECT `id`,`name`,`main_url`,`url`,`preg_one`,`preg_link`,`preg_name`,`preg_anonce`,`preg_img`,`preg_text`,`preg_f_img`,`active`,`rss` FROM `$row[0]` WHERE `id_wp`='$view_wp'";
		//echo "$qquurr";
		$sql_list_wwp=mysql_query($qquurr) or die ("Invalid:" . mysql_error());
		list($id_wh,$name_wh,$main_url,$url_wh,$preg_one,$preg_link,$preg_name,$preg_anonce,$preg_img,$preg_text,$preg_f_img,$active,$rss)=mysql_fetch_row($sql_list_wwp);
