...
$list_news=mysql_query("SELECT `name`,`main_url`,`url`,`preg_one`,`preg_link`,`preg_name`,`preg_anonce`,`preg_img`,`preg_text`,`preg_f_img` FROM `$_name_news_table` WHERE `active`='0'   ORDER BY `date_in` ASC ") or die ("Invalid:" . mysql_error());
...
//			print_r($HTTP);
		    ...
		    if(preg_match('#[а-яё]+#i',$url)) $url=my_url_encode($url); echo "<B>$url</B>";
		    ccurl($link_n[1]);
