...
function clean_name_line(string $line, ?string $forwarderCode, ?string $cellCode): string {
    ...
    $line = preg_replace(
        ////////'/^(to|an|von|from|empf[aä]nger|addressee|receiver|contact)\s*:?\s*/iu',
        '/^(to|an|von|from|empf[aä]nger|addressee|receiver|contact|billing\s+no)\s*:?\s*/iu',
        '',
        $line
    );
