...
function camex_az_connector_scenario_overrides(array $scenario): array
{
    ...
    foreach ($mapping as $overrideKey => $path) {
        $value = camex_az_connector_nested_value($scenario, $path);
        if ($value !== null && $value !== '') {
...
function camex_az_connector_cli_overrides(array $args): array
{
    ...
    foreach ($mapping as $argKey => $overrideKey) {
        ...
        if (in_array($overrideKey, ['login_path', 'login_post_path', 'dashboard_path'], true)) {
            $value = camex_az_connector_path($value);
        }
        $overrides[$overrideKey] = $value;
    }
...
{
    $lookup = camex_az_connector_find_lookup_arg($args);
    if ($lookup['type'] === '') {
        return null;
    }
...
{
    $scenario = camex_az_connector_decode_json_object($connectorRow['scenario_json'] ?? '');
    $overrides = array_merge(
        ...
        camex_az_connector_scenario_overrides($scenario),
        $connectorRow !== null ? camex_az_connector_row_overrides($connectorRow) : [],
        camex_az_connector_cli_overrides($args)
    );

    $overrides['base_url'] = rtrim((string)($overrides['base_url'] ?? ''), '/');
    foreach (['login_path', 'login_post_path', 'dashboard_path'] as $pathKey) {
        $overrides[$pathKey] = camex_az_connector_path((string)($overrides[$pathKey] ?? '/'));
    }
