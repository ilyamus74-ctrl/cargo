...
function camex_az_flight_list_usage(): string
{
    return <<<'TXT'
Usage:
  php run_sync_flight_list.php --connector-id=3 [options]

Options:
  --connector-id=ID          Load active connector by connectors.id.
  --connector-name=NAME      Load active connector by connectors.name.
  --connector-key=NAME       Load active connector by connectors.name (no connector_key column exists).
...
$argv = $_SERVER['argv'] ?? [];
$args = camex_az_connector_cli_args($argv);
...
try {
    $connectorRow = camex_az_connector_load_row($args, $repoRoot);
    $connectorDiagnostics = camex_az_connector_diagnostics($connectorRow);
    $overrides = camex_az_connector_build_forwarder_overrides($connectorRow, $args);
} catch (Throwable $e) {
...
$pagePath = camex_az_flight_list_normalize_page_path((string)($args['page-path'] ?? $args['page_path'] ?? '/cadmin/usa/index.php?do=flight'));
...
$correlationId = 'run-sync-flight-list-' . date('YmdHis') . '-' . bin2hex(random_bytes(4));
...
try {
    $result = camex_az_sync_flight_list_kernel([
        'repo_root' => $repoRoot,
