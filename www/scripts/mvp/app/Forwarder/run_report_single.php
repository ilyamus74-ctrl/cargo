 ...
 *   php run_report_single.php --base-url=... --login=... --password=... --track=... --session-file=/tmp/forwarder_session_13.json
...
function forwarder_single_normalize_base_url(string $rawBaseUrl): string
{
...
function forwarder_single_cli_overrides(array $argv): array
{
    $map = [
        ...
        'session-file' => 'FORWARDER_SESSION_FILE',
    ];
    ...
    foreach ($map as $argName => $envName) {
        ...
        if ($argName === 'base-url') {
            $normalized = forwarder_single_normalize_base_url($normalized);
        }
        $overrides[$envName] = $normalized;
    }
...
foreach ($overrides as $envName => $value) {
    ...
    if ($envName === 'FORWARDER_BASE_URL') {
        putenv('DEV_COLIBRI_BASE_URL=' . $value);
        $_ENV['DEV_COLIBRI_BASE_URL'] = $value;
        $_SERVER['DEV_COLIBRI_BASE_URL'] = $value;
    } elseif ($envName === 'FORWARDER_LOGIN') {
        putenv('DEV_COLIBRI_LOGIN=' . $value);
        $_ENV['DEV_COLIBRI_LOGIN'] = $value;
        $_SERVER['DEV_COLIBRI_LOGIN'] = $value;
    } elseif ($envName === 'FORWARDER_PASSWORD') {
        putenv('DEV_COLIBRI_PASSWORD=' . $value);
        $_ENV['DEV_COLIBRI_PASSWORD'] = $value;
        $_SERVER['DEV_COLIBRI_PASSWORD'] = $value;
    }
}
