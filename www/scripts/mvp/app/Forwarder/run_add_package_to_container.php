...
function forwarder_add_package_to_container_render_template_html(string $template, array $vars): string
{
    $replace = [];
...
function forwarder_add_package_to_container_build_html_label_from_verify(
    ?array $verifyJson,
    string $track,
...
): array
{
    ...
    if (!is_array($verifyJson)) {
        return ['ok' => false, 'error' => 'verify json is empty', 'label_base64' => '', 'file_name' => '', 'template_code' => $templateCode];
    }
    ...
    $amountCurrencyRaw = trim((string)($package['amount_currency'] ?? 'EUR'));
    $categoryRaw = trim((string)($package['category'] ?? $package['title'] ?? ''));

    $sellerRaw = forwarder_add_package_to_container_pick_first_non_empty([
        $package['seller_title'] ?? '',
        $package['seller'] ?? '',
        $verifyJson['seller_title'] ?? '',
        $verifyJson['seller'] ?? '',
    ]);

    $invoiceEurRaw = forwarder_add_package_to_container_pick_first_non_empty([
        $package['invoice'] ?? '',
        $package['invoice_price'] ?? '',
        $verifyJson['invoice'] ?? '',
        $verifyJson['invoice_price'] ?? '',
    ]);

    $invoiceUsdRaw = $invoiceEurRaw !== '' ? $invoiceEurRaw : '0.00';
...
$countryDestNorm = strtoupper(trim($countryDestRaw));
$countryDestRaw = match ($countryDestNorm) {
    'AZ', 'AZB', 'AZE' => 'Azerbaijan',
    'GE', 'GEO', 'TB', 'TBS' => 'Georgia',
    'KG', 'KGZ' => 'Kyrgyzstan',
    'DE', 'DEU' => 'Germany',
    default => $countryDestRaw !== '' ? $countryDestRaw : 'Azerbaijan',
};
    ...
    $seller = htmlspecialchars($sellerRaw, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
    ...
    $amountNumeric = '0.00';
    $invoiceEurNumeric = 0.0;
    if (preg_match('/([0-9]+(?:\.[0-9]+)?)/', $invoiceEurRaw, $invoiceMatch) === 1) {
       $invoiceEurNumeric = (float)$invoiceMatch[1];
    }

    $amountEurNumeric = 0.0;
    if (preg_match('/([0-9]+(?:\.[0-9]+)?)/', $amountRaw, $amountMatch) === 1) {
       $amountEurNumeric = (float)$amountMatch[1];
    }

    $totalWaybillInvoicePrice = htmlspecialchars(
        number_format($invoiceEurNumeric + $amountEurNumeric, 2, '.', '') . ' EUR',
        ENT_QUOTES | ENT_SUBSTITUTE,
        'UTF-8'
    );

    $defaultTemplate = '<!doctype html><html><head><meta charset="utf-8"><title>Waybill {{track}}</title>'
        . '<style>'
    ...
    $labelVars = [
        ...
        'barcode_url' => htmlspecialchars($barcodeUrl, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'),
        'qr_img_html' => $qrImg,
    ...
    $fallbackLines = [
        'WAYBILL',
        'TRACK: ' . $track,
        'CLIENT: ' . $clientNameRaw . ' ' . $clientCodeRaw,
        'CLIENT ID: ' . $clientIdRaw,
        'INTERNAL ID: ' . $internalIdRaw,
        'WEIGHT: ' . ($weightRaw !== '' ? $weightRaw : '0.000'),
        'VOLUME WEIGHT: ' . ($volumeWeightRaw !== '' ? $volumeWeightRaw : '0.000'),
        'AMOUNT: ' . ($amountRaw !== '' ? $amountRaw : ('0.00 ' . $amountCurrencyRaw)),
        'FLIGHT: ' . $flightNameRaw,
        'FROM/TO: ' . $flightDepartureRaw . ' / ' . $flightDestinationRaw,
        'PHONE: ' . $clientPhoneRaw,
        'ADDRESS: ' . $clientAddressRaw,
        'INVOICE URL: ' . $invoiceUrl,
    ];
    $fallbackPdf = forwarder_add_package_to_container_build_simple_pdf($fallbackLines);
...
//        'file_name' => 'waybill_' . preg_replace('/[^a-zA-Z0-9_-]+/', '_', $track) . '.html',
        'error' => 'html->pdf render unavailable, simple pdf fallback used: ' . (string)($pdfRender['error'] ?? ''),
...
$forwardNameArg = trim(forwarder_add_package_to_container_arg($args, 'forward-name', 'forward_name'));
$countryDestArg = trim(forwarder_add_package_to_container_arg($args, 'country-dest', 'country_dest'));
...
$verifiedPackage = is_array($verifyJson['package'] ?? null) ? $verifyJson['package'] : [];
$verifiedContainerId = trim((string)($verifiedPackage['container_id'] ?? ''));
$alreadyInSelectedContainer = $position !== ''
    && $verifiedContainerId !== ''
    && $verifiedContainerId === trim((string)$position);
/*
 * Некоторые форварды, например ASER, могут возвращать 404 на check-position,
 * но успешно выполнять change-position.
 * Для фактической отгрузки главным считаем результат change-position.
 */
...
if ($printRequested) {
    ...
    if ($allowLabelUrl && $labelBase64 === '' && $labelUrl !== '') {
    ...
    if (
        ...
        $printResponsePayload['message'] = !empty($printResponsePayload['ok'])
            ? 'print job queued'
            : (string)($printResponsePayload['error'] ?? 'print job failed');
