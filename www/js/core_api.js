...
const CoreAPI = {
    ...
    forms: {
        ...
        build(action, link) {
            const builders = {
                ...
                'warehouse_stock_history_modal': () => this.withAttribute('item_id', link),
                'warehouse_stock_history_forwarder_reports': () => {
                    const fd = new FormData();
                    fd.append('item_id', link?.getAttribute('data-item-id') || '');
                    fd.append('limit', link?.getAttribute('data-limit') || '2000');
                    return fd;
                },
                'warehouse_stock_register_forwarder': () => {
                    const fd = new FormData();
                    const modalItemId = document.querySelector('#item-stock-modal-form input[name="item_id"]')?.value || '';
                    const dataItemId = link?.getAttribute('data-item-id') || '';
                    const force = link?.getAttribute('data-force') || '';
                    fd.append('item_id', dataItemId || modalItemId);
                    if (force) {
    ...
    ui: {
        ...
        loadMain(html) {
            ...
            if (main) {
                ...
                if (CoreAPI.warehouseItemsRegistry?.init) {
                    CoreAPI.warehouseItemsRegistry.init();
                }
    ...
    handlers: {
        ...
        'open_connector_label_template_print_preview': (data) => {
            ...
            const initialRotate = [0, 90, 180, 270].includes(rotate) ? rotate : 0;
            ...
            const doc = [
                ...
                '.label-print-page { margin: 0; padding: 0; box-sizing: border-box; overflow: hidden; background: #fff; }',
                '.label-print-content { margin: 0; padding: 0; transform-origin: top left; }',
                '.label-native-layout { width: 100%; height: 100%; overflow: hidden; transform: none; }',
                '.label-print-page, .label-print-page * { box-sizing: border-box; }',
                '@media screen {',
                '  body { background: #e9ecef; padding: 12px; }',
                '  .print-toolbar { position: sticky; top: 0; z-index: 10; margin-bottom: 12px; padding: 8px; background: #fff; border: 1px solid #ddd; font-family: Arial, sans-serif; font-size: 13px; }',
                '  .print-toolbar label { display: inline-flex; align-items: center; gap: 4px; margin: 2px 8px 2px 0; white-space: nowrap; }',
                '  .print-toolbar input, .print-toolbar select { width: 74px; font: inherit; }',
                '  .print-toolbar select { width: auto; }',
                '  .print-toolbar button { margin: 2px 4px 2px 0; }',
                '  .print-warning { margin-top: 6px; color: #b54708; font-weight: 600; }',
                '  .print-diagnostics { margin-top: 6px; color: #495057; font-family: Consolas, monospace; font-size: 12px; }',
                '  .label-print-page { outline: 1px dashed #999; background: #fff; }',
                '}',
                '@media print {',
                '  html, body { margin: 0 !important; padding: 0 !important; overflow: hidden !important; background: #fff !important; }',
                '  .print-toolbar { display: none !important; }',
                '  .label-print-page { position: fixed; left: 0; top: 0; margin: 0 !important; padding: 0 !important; overflow: hidden !important; outline: none !important; page-break-after: avoid; break-after: avoid; }',
                '  .label-print-content { margin: 0 !important; }',
                '}',
                '</style>',
                '</head>',
                '<body>',
                ...
                '<button type="button" data-preset-width="150" data-preset-height="100">150 x 100 mm</button>',
                '<button type="button" data-preset-width="100" data-preset-height="90">100 x 90 mm</button>',
                '<button type="button" data-preset-width="102" data-preset-height="203">102 x 203 mm</button>',
                '</div>',
                '<div class="print-warning">Paper size must match: 100 x 150 mm / 4 x 6, Scale 100%, Margins None, Headers off. Не используйте 4 x 8.</div>',
                '<div class="print-warning" style="font-weight:400;">Rotate in browser preview is visual only; printer rotation is handled separately.</div>',
                '<div id="label-print-diagnostics" class="print-diagnostics"></div>',
                toolbarValidation.join(''),
                '</div>',
                '<div class="label-print-page label-print-css-override" id="label-print-page">',
                '<div class="label-print-content label-native-layout" id="label-print-content">',
                printableHtml,
                '</div>',
                '</div>',
                '<script>',
                ...
                '    return {',
                '      titleFontPx: clamp(Math.round(shortSide * 0.24), 20, 34),',
                '      baseFontPx: clamp(Math.round(shortSide * 0.12), 9, 16),',
                '      smallFontPx: clamp(Math.round(shortSide * 0.09), 7, 12),',
                '      cellPaddingPx: clamp(Math.round(shortSide * 0.018), 1, 3),',
                '      borderWidthPx: 1,',
                '      barcodeHeightMm: clamp(Math.round(heightMm * 0.16), 14, 28),',
                '    };',
                '  }',
                ...
                '    const widthMm = Math.max(1, numberValue("label-width-mm", 100));',
                '    const heightMm = Math.max(1, numberValue("label-height-mm", 150));',
                '    const vars = computeLabelCssVars(widthMm, heightMm);',
                '    const layoutMode = $("label-layout-mode")?.value || "native-css";',
                '    const cssOverride = Boolean($("label-css-override")?.checked);',
                '    const manualScale = Math.max(1, numberValue("label-scale-percent", 100));',
                '    const offsetX = numberValue("label-offset-x-mm", 0);',
                '    const offsetY = numberValue("label-offset-y-mm", 0);',
                '    const fitMode = $("label-fit-mode")?.value || "contain";',
                '    const rotate = Number($("label-rotate")?.value || 0);',
                '    updateDynamicStyle(widthMm, heightMm, vars);',
                '    page.classList.toggle("label-print-css-override", cssOverride);',
                '    content.classList.toggle("label-native-layout", layoutMode === "native-css");',
                '    content.style.transform = "none";',
                '    content.style.transformOrigin = "top left";',
                '    const pageWidthPx = page.clientWidth;',
                '    const pageHeightPx = page.clientHeight;',
                '    const contentWidthPx = Math.max(1, content.scrollWidth);',
                '    const contentHeightPx = Math.max(1, content.scrollHeight);',
                '    const scaleContain = Math.min(pageWidthPx / contentWidthPx, pageHeightPx / contentHeightPx);',
                '    const scaleCover = Math.max(pageWidthPx / contentWidthPx, pageHeightPx / contentHeightPx);',
...
'test_print_connector_label_template': (data) => {
    if (data?.print_mode === 'zpl_vector_template' || data?.print_mode === 'zpl_raw' || data?.print_mode === 'pdf_template') {
        ...
        if (box) {
            ...
            box.innerHTML = `<div class="${ok ? 'text-success' : 'text-danger'}"><strong>${esc(message)}</strong></div>`
                + `<div class="small text-muted mt-1">print_mode=${esc(data?.print_mode || '')}; transport=${esc(data?.zpl_transport || data?.diagnostics?.cups_queue || '')}; size=${esc(data?.zpl_size || data?.diagnostics?.pdf_size || '')}</div>`
    ...
    alert(message);
        ...
        'open_item_in_batch': (data) => {
            ...
            if (typeof initWarehouseCellPersistence === 'function') {
                initWarehouseCellPersistence();
            }
        ...
        'save_item_stock': (data) => {
            ...
            CoreAPI.ui.onModalCloseOnce(() => {
                ...
                if (CoreAPI.warehouseItemsRegistry?.resetAndLoad) {
                    CoreAPI.warehouseItemsRegistry.resetAndLoad();
                }
            });
            CoreAPI.ui.closeModal();
        },

        'warehouse_stock_history_modal': (data) => {
            console.log('warehouse_stock_history_modal response:', data);
            CoreAPI.ui.showModal(data.html);
        },

        'warehouse_stock_history_forwarder_reports': (data) => {
            console.log('warehouse_stock_history_forwarder_reports response:', data);

            const box = document.getElementById('warehouse-stock-history-forwarder-reports');
            if (box) {
                box.innerHTML = data.html || '<div class="alert alert-secondary">Репорты не найдены</div>';
            }
        },

        'warehouse_stock_register_forwarder': async (data, link, formData) => {
            alert(data.message || 'Регистрация у форварда выполнена');

            const itemId = String(data.item_id || formData.get('item_id') || '').trim();

            if (CoreAPI.warehouseItemsRegistry?.resetAndLoad) {
                await CoreAPI.warehouseItemsRegistry.resetAndLoad();
            }

            const modalOpen = document.querySelector('#fullscreenModal.show');
            if (modalOpen && itemId) {
                const fd = new FormData();
                fd.append('action', 'open_item_stock_modal');
                fd.append('item_id', itemId);
                const modalData = await CoreAPI.client.call(fd);
                if (modalData?.status === 'ok' && modalData.html) {
                    CoreAPI.ui.showModal(modalData.html);
                    if (typeof initStandDevicePersistence === 'function') {
                        initStandDevicePersistence();
                    }
                    if (typeof initWarehouseStockAddons === 'function') {
    ...
    events: {
        ...
        async handleClick(e) {
            ...
            if (action === 'upload_item_stock_photo') {
                const existingPhotoType = link.getAttribute('data-photo-type') || 'label';
                if (typeof openWarehousePhotoViewer === 'function' && openWarehousePhotoViewer(existingPhotoType)) {
                    return;
                }
            ...
            if (action === 'warehouse_stock_register_forwarder') {
                const force = link.getAttribute('data-force') === '1';
                const message = force
                    ? 'Выполнить регистрацию посылки у форварда повторно с текущими сохранёнными данными? Если меняли поля в карточке — сначала нажмите “Сохранить”.'
                    : 'Запустить регистрацию посылки у форварда? Перед запуском убедитесь, что изменения в карточке сохранены.';
                if (!confirm(message)) {
                    return;
                }
            }
        ...
        async handleWarehouseItemStockPhotoUpload(e) {
            ...
            try {
                ...
                if (photoType === 'label' && typeof renderWarehouseLabelPhotoPreview === 'function') {
                    renderWarehouseLabelPhotoPreview(data.json || '');
                }
            } catch (err) {
    ...
    // ====================================
    // WAREHOUSE - Unified parcels registry
    // ====================================
    warehouseItemsRegistry: {
        ...
        init() {
            const root = document.getElementById('warehouse-items-registry');
            if (!root) return;
            if (this.initialized && this.root === root) {
                this.resetAndLoad();
                return;
            ...
            this.root = root;
            this.tbody = root.querySelector('#warehouse-items-registry-tbody');
            this.total = root.querySelector('#warehouse-items-registry-total');
        ...
        bindEvents() {
            if (this.root?.dataset.registryBound === '1') {
                return;
            }
            this.root.dataset.registryBound = '1';

            const bindSelect = (element, stateKey, fallback) => {
                element.addEventListener('change', () => {
                    this.state[stateKey] = element.value || fallback;
                    this.resetAndLoad();
                });
            };
        ...
        async loadNext() {
            ...
            try {
                const data = await CoreAPI.client.call(fd);
                if (!data || data.status !== 'ok') {
                    console.error('core_api error (warehouse_items_registry):', data);
                    return;
                }
                if (this.state.offset === 0) {
                    this.tbody.innerHTML = data.html || '';
                ...
                console.error('core_api fetch error (warehouse_items_registry):', err);
            } finally {
                this.setLoading(false);
            }
        }
...
- ${missingVars.map((item) => String(item)).join('\n- ')}`);
    ...
    warehouseItemOut: {
        ...
        autoConfirmInFlight: false,
        autoConfirmedTracking: '',
        lastConfirmedTracking: '',
        suppressDuplicateScanUntil: 0,
        ...
        playOutcomeSound(type = 'success') {
            const src = type === 'success' ? '/alert_sound/accept.mp3' : '/alert_sound/alert.mp3';
            try {
                const audio = new Audio(src);
                audio.play().catch(() => {});
            } catch (e) {}
        },
        scheduleAutoConfirm(trackingNo = '') {
            if (this.autoConfirmTimer) {
                clearTimeout(this.autoConfirmTimer);
            }
            if (this.autoConfirmInFlight) {
                return;
            }
            const normalizedTracking = String(trackingNo || '').trim();
            if (normalizedTracking !== '' && this.autoConfirmedTracking === normalizedTracking) {
                return;
            }
            this.autoConfirmTimer = window.setTimeout(() => {
                if (!this.modalConfirmButton || this.modalConfirmButton.classList.contains('d-none') || this.modalConfirmButton.disabled) {
                    return;
                }
                this.autoConfirmInFlight = true;
                if (normalizedTracking !== '') {
                    this.autoConfirmedTracking = normalizedTracking;
                }
                this.confirmSend({ auto: true });
            }, 120);
        },
        ...
        async processTrackingInput() {
            ...
            if (
                this.lastConfirmedTracking !== '' &&
                trackingNo === this.lastConfirmedTracking &&
                Date.now() < this.suppressDuplicateScanUntil
            ) {
                return;
            }
            this.openModal();
        ...
        renderLookupResult(item, requestedTracking, fallbackMessage = '') {
            ...
            if (normalizedStatus === 'to_send' || normalizedStatus === 'sended') {
                ...
                if (containerMeta) {
                    this.scheduleAutoConfirm(trackLabel);
                }
                return;
            }
        ...
        async confirmSend({ auto = false } = {}) {
            if (!this.currentLookupItem || this.state.lookupLoading || this.confirmInFlight) {
                return;
            }
            ...
            this.playOutcomeSound('success');
            this.lastConfirmedTracking = String(this.currentLookupItem?.tracking_no || this.currentLookupItem?.tuid || '').trim();
            this.suppressDuplicateScanUntil = Date.now() + 5000;
            this.currentLookupItem = null;
            this.state.search = '';
            if (this.searchInput) {
                this.searchInput.value = '';
    ...
    departures: {
        ...
        buildFlightRuntimeVars(button) {
            ...
            const carrierAttr = String(button?.getAttribute('data-carrier') || '').trim();
            const carrierFromFlightTimeMatch = flightTime.match(/^[A-Za-z]{3}/);
            const carrierFromFlightTime = carrierFromFlightTimeMatch ? String(carrierFromFlightTimeMatch[0] || '').toUpperCase() : '';
            const carrier = carrierAttr || departure || carrierFromFlightTime;
    ...
    init() {
        ...
        this.warehouseItemsRegistry.init();
        this.warehouseWithoutAddons.init();
        this.warehouseInStorage.init();
        this.warehouseSync.init();
        this.warehouseSyncReports.init();
...
function initWarehouseStockAddons() {
    ...
    function createSelect(addonKey, optionsMap, initialValue) {
        ...
        sortedOptionEntries(addonKey, optionsMap).forEach(function (entry) {
            var opt = document.createElement('option');
            opt.value = entry.value;
            opt.textContent = entry.label;
            if (String(initialValue || '') === entry.value) {
                opt.selected = true;
            }
...
function normalizeForwarderClientName(value) {
    return String(value || '')
        .trim()
        .replace(/\s+/g, ' ');
}

function enforceForwarderClientName(receiverName) {
    if (!receiverName) return;

    var locked = receiverName.dataset.forwarderLookupLocked === '1';
    var exactName = normalizeForwarderClientName(receiverName.dataset.forwarderLookupClientName || '');

    if (!locked || !exactName) {
        return;
    }

    var current = normalizeForwarderClientName(receiverName.value || '');

    if (current !== exactName) {
        receiverName.value = exactName;
        receiverName.dispatchEvent(new Event('input', { bubbles: true }));
        receiverName.dispatchEvent(new Event('change', { bubbles: true }));
    }
}

function clearForwarderClientNameLock(receiverName) {
    if (!receiverName) return;

    receiverName.dataset.forwarderLookupLocked = '';
    receiverName.dataset.forwarderLookupClientName = '';
    receiverName.dataset.forwarderLookupClientId = '';
}

function normalizeForwarderClientCellCode(value) {
    ...
    // OCR/ручной ввод иногда даёт похожие варианты
    v = v.replace(/^АS/i, 'AS'); // кириллическая А + S, если попадётся

    return v;
}

function looksLikeForwarderClientCode(value) {
    var v = normalizeForwarderClientCellCode(value);
...
function initForwarderClientLookup() {
    ...
    function clearLookupLock() {
        clearForwarderClientNameLock(nameInput);
    }

    async function runLookup() {
        var normalizedAddress = normalizeForwarderClientCellCode(addressInput.value);

        if (normalizedAddress && addressInput.value !== normalizedAddress) {
            addressInput.value = normalizedAddress;
            addressInput.dispatchEvent(new Event('input', { bubbles: true }));
            addressInput.dispatchEvent(new Event('change', { bubbles: true }));
        }

        var receiverAddress = normalizedAddress;
        ...
        try {
            ...
            var clientName = normalizeForwarderClientName(data.client_name);
            if (clientName) {
                nameInput.value = clientName;
                nameInput.dataset.forwarderLookupLocked = '1';
                nameInput.dataset.forwarderLookupClientName = clientName;
                nameInput.dataset.forwarderLookupClientId = String(data.client_id || '');
                nameInput.title = 'Уточнено по базе клиентов форварда, client_id=' + (data.client_id || '');
                nameInput.dispatchEvent(new Event('input', { bubbles: true }));
                nameInput.dispatchEvent(new Event('change', { bubbles: true }));
                showLookupToast(clientName);

                setTimeout(function () { enforceForwarderClientName(nameInput); }, 100);
                setTimeout(function () { enforceForwarderClientName(nameInput); }, 500);
                setTimeout(function () { enforceForwarderClientName(nameInput); }, 1200);
            }
        } catch (err) {
    ...
    ['change', 'input', 'blur'].forEach(function (eventName) {
        companyInput.addEventListener(eventName, function () {
            syncReceiverCountryFromForwarder();
            clearLockAndScheduleLookup();
            setTimeout(scheduleLookup, 50);
            setTimeout(scheduleLookup, 300);
        });
...
function initItemInScanEntryBehaviour() {
    var form = document.getElementById('item-in-modal-form');
    if (!form) return;
    ...
    if (!form.__itemInTrackMirrorBound) {
        form.__itemInTrackMirrorBound = true;
        var isSyncing = false;
        ...
        trackingInput.addEventListener('input', function () {
            syncPair(trackingInput, tuidInput);
        });
        trackingInput.addEventListener('change', function () {
            syncPair(trackingInput, tuidInput);
        });
        tuidInput.addEventListener('input', function () {
            syncPair(tuidInput, trackingInput);
        });
        tuidInput.addEventListener('change', function () {
            syncPair(tuidInput, trackingInput);
        });
...
function initWarehouseCellPersistence() {
    var select = document.getElementById('cellId');
    if (!select || !window.localStorage) return;
    if (select.__cellPersistenceBound) return;
    select.__cellPersistenceBound = true;

    var storageKey = 'warehouse_item_in_last_cell_id';
    var storedValue = localStorage.getItem(storageKey);
    if (storedValue) {
        var hasOption = Array.prototype.some.call(select.options, function (opt) {
            return opt.value === storedValue;
...
function getWarehousePhotoPaths(photoType) {
    var hiddenId = photoType === 'box'
        ? 'warehouseStockBoxImageJson'
        : 'warehouseStockLabelImageJson';

    var input = document.getElementById(hiddenId);
    var raw = input ? String(input.value || '').trim() : '';

    if (!raw) {
        var btnId = photoType === 'box'
            ? 'warehouseStockTakeBoxPhotoBtn'
            : 'warehouseStockTakeLabelPhotoBtn';

        var btn = document.getElementById(btnId);
        raw = btn ? String(btn.getAttribute('data-photo-json') || '').trim() : '';
    }
...
function openWarehousePhotoViewer(photoType) {
    var paths = getWarehousePhotoPaths(photoType);
    if (!paths.length) return false;

    if (paths.length === 1) {
        window.open(paths[0], '_blank', 'noopener');
        return true;
    }

    var html = '<div class="d-flex flex-wrap gap-2">';
    paths.forEach(function (path) {
        var safe = String(path || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
        html += '<a href="' + safe + '" target="_blank" rel="noopener" class="d-inline-block border rounded p-1">'
              + '<img src="' + safe + '" alt="Фото" style="max-width:180px;max-height:140px;object-fit:contain;">'
              + '</a>';
    });
    html += '</div>';

    if (CoreAPI.ui && typeof CoreAPI.ui.showModal === 'function') {
        CoreAPI.ui.showModal(
            '<div class="modal-header">'
            + '<h5 class="modal-title">Фото посылки</h5>'
...
function updateWarehousePhotoButtonState(photoType, jsonRaw) {
    var isBox = photoType === 'box';

    var btn = document.getElementById(isBox ? 'warehouseStockTakeBoxPhotoBtn' : 'warehouseStockTakeLabelPhotoBtn');
    var info = document.getElementById(isBox ? 'warehouseStockBoxPhotoInfo' : 'warehouseStockLabelPhotoInfo');

    var paths = [];
    try {
        paths = JSON.parse(jsonRaw || '[]');
    } catch (e) {
        paths = [];
    }

    if (btn && Array.isArray(paths) && paths.length > 0) {
        btn.setAttribute('data-photo-json', JSON.stringify(paths));
        btn.textContent = isBox ? 'Открыть Фото содерж' : 'Открыть Фото Лейб';
        btn.classList.remove('opacity-50');
        btn.disabled = false;
    } else if (btn) {
        btn.setAttribute('data-photo-json', '[]');
        btn.textContent = isBox ? 'Фото содерж' : 'Фото Лейб';
    }

    if (info && Array.isArray(paths) && paths.length > 0) {
        info.textContent = isBox ? 'Фото содержимого загружено' : 'Фото лейбла загружено';
    } else if (info) {
        info.textContent = '';
    }
}

function renderWarehouseLabelPhotoPreview(jsonRaw) {
    var box = document.getElementById('warehouseStockLabelPhotoPreview');
    var hidden = document.getElementById('warehouseStockLabelImageJson');

    if (!box) {
        box = document.createElement('div');
        box.id = 'warehouseStockLabelPhotoPreview';
        box.className = 'mt-2 d-flex flex-wrap gap-2';
    ...
    var paths = [];
    try {
        paths = JSON.parse(jsonRaw || (hidden ? hidden.value : '') || '[]');
    } catch (e) {
        paths = [];
    }
    ...
    box.innerHTML = paths.map(function (path) {
        var safe = String(path || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
        return '<a href="' + safe + '" target="_blank" rel="noopener" class="d-inline-block border rounded p-1">'
             + '<img src="' + safe + '" alt="Фото лейбла" style="max-width:120px;max-height:90px;object-fit:contain;">'
             + '</a>';
    }).join('');
}
...
function renderWarehousePhotoPreviewButtons(photoType, paths) {
    updateWarehousePhotoButtonState(photoType, Array.isArray(paths) && paths.length ? JSON.stringify(paths) : '');
}
...
(function installDeviceFlowApi(){
  ...
  window.clearWarehouseItemOutSearch = function () {
    const el = document.getElementById('warehouse-item-out-search');
    if (!el) return false;
    el.value = '';
    el.dispatchEvent(new Event('input', { bubbles: true }));
    el.dispatchEvent(new Event('change', { bubbles: true }));
