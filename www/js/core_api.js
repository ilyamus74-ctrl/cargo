...
CoreAPI.pageInits.warehouse_move = function warehouseMoveInit() {
    ...
    if (window.__warehouseMoveInitInstalled) return;
    window.__warehouseMoveInitInstalled = true;

window.__dbgConfirmScanner = function() {
  showDebug('CONFIRM: scanner context', true);
  return true;
};

window.__dbgConfirmModal = function() {
  showDebug('CONFIRM: scanner_modal context', true);
  return true;
};

    // --- debug helper (используем твой блок #scanner-debug-status) ---
    function showDebug(message, isError = false) {
        ...
        if (debugDiv && debugText) {
            ...
            debugText.textContent = String(message);
...
window.openMoveModal = async function() {
  try {
    ...
    window.setCellFromQR = function (qrValue) {
        ...
        const trySet = (tries = 5) => {
            ...
            if (!cellSelect) {
                ...
                showDebug('setCellFromQR: #cellId not found', true);
                return false;
            }
            ...
            if (!found) {
                showDebug('setCellFromQR: cell not found ' + cellCode, true);
                return false;
            }
            ...
            showDebug('setCellFromQR: OK ' + found.text);
            return true;
        };
    ...
    window.confirmBatchMove = function() {
        ...
        if (!row) {
            showDebug('⚠️ Строка результата не найдена', true);
            return false;
        }

        const button = row.querySelector('.js-warehouse-move-batch-action');
        const itemLink = row.querySelector('[data-item-id]');
        const itemId = button?.getAttribute('data-item-id') || itemLink?.getAttribute('data-item-id');

        if (!itemId) {
            showDebug('❌ Не найден data-item-id', true);
            return false;
        }
        ...
        CoreAPI.client.call(formData)
            .then((data) => {
                ...
                const handler = CoreAPI.handlers?.warehouse_move_batch_assign || CoreAPI.handlers?.default;
                if (handler) {
                    handler(data);
                }
            })
...
if (!window.__deviceFlowApiInstalled) {
    ...
    // Note: Uses recursive setTimeout pattern because it needs to return synchronously
    // while still retrying internally. This is different from the Android retry pattern
    // which can be fully async.
    window.setCellFromQR = function(qrValue) {
    ...
    window.confirmBatchMove = function() {
        fallbackShowDebug('📦 confirmBatchMove: проверка (fallback)', true);
        try {
            const cellSelect = document.getElementById('warehouse-move-batch-cell');
            if (!cellSelect || !cellSelect.value) {
                alert('Выберите ячейку склада');
            ...
            const tbody = document.getElementById('warehouse-move-batch-results-tbody');
            if (!tbody) {
                fallbackShowDebug('❌ Таблица результатов не найдена', true);
                return false;
            }
            ...
            const total = Number((totalEl?.textContent || '').trim() || 0);
            if (total !== 1) {
                fallbackShowDebug('⚠️ Нужно ровно одно совпадение', true);
                return false;
            }

            const row = tbody.querySelector('tr');
            if (!row) {
                fallbackShowDebug('⚠️ Строка результата не найдена', true);
                return false;
            }
            ...
            if (!itemId) {
                fallbackShowDebug('❌ Не найден data-item-id', true);
                return false;
            }

            if (button && button instanceof HTMLButtonElement && button.disabled) {
                fallbackShowDebug('❌ Кнопка перемещения недоступна', true);
                return false;
            }

            fallbackShowDebug('✅ Перемещаем посылку', false);
            ...
            console.error('confirmBatchMove error:', e);
            fallbackShowDebug('confirmBatchMove error', true);
            return false;
        }
    };
...
function showOverlayDebug(message, isError = false) {
  console.log(message);

  let box = document.getElementById('__wm_overlay_debug');
  if (!box) {
    box = document.createElement('div');
    box.id = '__wm_overlay_debug';
  ...
  box.style.background = isError ? '#fff3cd' : '#d1ecf1';
  box.style.borderColor = isError ? '#ffc107' : '#0dcaf0';
  box.textContent = String(message);

  // можно автоскрывать, но для отладки лучше оставить
}
window.__wmOverlayDebug = showOverlayDebug;

window.__dbgConfirmScanner = function() {
  window.__wmOverlayDebug('CONFIRM fired: scanner', true);
  return true;
};
window.__dbgConfirmModal = function() {
  window.__wmOverlayDebug('CONFIRM fired: scanner_modal', true);
  return true;
};

window.__wmOverlayDebugConfirm = function() {
  alert('CONFIRM FIRED');
  return true;
}
