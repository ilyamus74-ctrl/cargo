...
function system_tasks_run_one(mysqli $dbcnx, int $taskId, int $systemUserId = 0): array
{
    ...
    try {
        ...
        if (is_array($result['context'] ?? null)) {
            ...
            foreach (['errors', 'connector_id', 'connector_ids', 'rows_total', 'report_items_upserted', 'stock_created', 'stock_updated', 'mapped_to_cells', 'unmapped_positions', 'out_sync_attempted', 'out_created_to_send', 'out_updated_to_send', 'out_existing_to_send', 'out_skipped_status', 'out_skipped_existing_state', 'out_errors'] as $contextKey) {
...
function system_tasks_run_forwarder_report_import(mysqli $dbcnx, array $task, int $systemUserId = 0): array
{
    ...
    foreach (array_keys($connectorIds) as $connectorId) {
        ...
        foreach (['rows_total', 'report_items_upserted', 'stock_created', 'stock_updated', 'mapped_to_cells', 'unmapped_positions', 'out_sync_attempted', 'out_created_to_send', 'out_updated_to_send', 'out_existing_to_send', 'out_skipped_status', 'out_skipped_existing_state', 'out_errors'] as $key) {
...
function system_tasks_forwarder_report_import_message(array $summary): string
{
    return 'forwarder_report_import done; connector_id=' . (string)($summary['connector_id'] ?? implode(',', (array)($summary['connector_ids'] ?? [])))
        ...
        . '; out_sync_attempted=' . (int)($summary['out_sync_attempted'] ?? 0)
