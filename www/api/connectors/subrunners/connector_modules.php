...
function connectors_subrunner_upsert_container_row(
    ...
    $sql = "
        ...
        ON DUPLICATE KEY UPDATE
            ...
            awb = VALUES(awb),
            packages_count = CASE
                WHEN VALUES(packages_count) IS NULL OR VALUES(packages_count) <= 0 THEN packages_count
                ELSE VALUES(packages_count)
            END,
            total_weight = CASE
                WHEN VALUES(total_weight) IS NULL OR VALUES(total_weight) <= 0 THEN total_weight
                ELSE VALUES(total_weight)
            END,
            compare_status = CASE
                WHEN warehouse_packages_count IS NULL OR warehouse_total_weight IS NULL THEN 'pending'
                WHEN warehouse_packages_count = CASE
                        WHEN VALUES(packages_count) IS NULL OR VALUES(packages_count) <= 0 THEN packages_count
                        ELSE VALUES(packages_count)
                    END
                     AND ABS(warehouse_total_weight - CASE
                        WHEN VALUES(total_weight) IS NULL OR VALUES(total_weight) <= 0 THEN total_weight
                        ELSE VALUES(total_weight)
                    END) < 0.0005 THEN 'matched'
                ELSE 'mismatch'
            END,
            ...
            compare_error = CASE
                ...
                ELSE CONCAT(
                    'warehouse=',
                    COALESCE(warehouse_packages_count, 'null'),
                    '/',
                    COALESCE(warehouse_total_weight, 'null'),
                    '; forwarder=',
                    COALESCE(CASE
                        WHEN VALUES(packages_count) IS NULL OR VALUES(packages_count) <= 0 THEN packages_count
                        ELSE VALUES(packages_count)
