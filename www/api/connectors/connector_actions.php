...
function connectors_departures_table_exists(mysqli $dbcnx, string $tableName): bool
{
...
function connectors_departures_resolve_table_names(array $connector): array
{
    $tableNames = [];
    ...
    foreach ($runtimeOperations as $operation) {
        ...
        try {
            ...
            error_log('connectors departures resolve table error: ' . $e->getMessage());
        }
    }
    ...
    if ($tableNames === []) {
        try {
            ...
            error_log('connectors departures resolve default table error: ' . $e->getMessage());
        }
    }
...
function connectors_departures_status_badge_class(string $status): string
{
...
function connectors_departures_format_value($value, int $decimals = 3): string
{
    if ($value === null) {
        return '—';
    }
...
function connectors_departures_format_datetime(?string $value): string
{
...
function connectors_departures_decode_containers($rawContainers): array
{
    ...
    foreach ($decoded as $container) {
        ...
        $containers[] = [
            ...
            'packages_count' => connectors_departures_format_value($container['packages_count'] ?? null, 0),
            'total_weight' => connectors_departures_format_value($container['total_weight'] ?? null),
        ];
    }
...
function connectors_departures_fetch_rows(mysqli $dbcnx, array $connector, string $statusFilter = 'ALL'): array
{
    ...
    foreach (connectors_departures_resolve_table_names($connector) as $tableName) {
        if (!connectors_departures_table_exists($dbcnx, $tableName)) {
            continue;
        }
        ...
        $stmt = $dbcnx->prepare($sql);
        if (!$stmt) {
            error_log('connectors departures prepare error: ' . $dbcnx->error . '; table=' . $tableName);
            continue;
        }

        $stmt->bind_param('i', $connectorId);
        if (!$stmt->execute()) {
            error_log('connectors departures execute error: ' . $stmt->error . '; table=' . $tableName);
            $stmt->close();
            continue;
        }
        ...
        if ($res instanceof mysqli_result) {
            while ($flight = $res->fetch_assoc()) {
                ...
                $containers = connectors_departures_decode_containers($flight['containers_json'] ?? '');
                ...
                $rows[] = [
                    ...
                    'status_badge_class' => connectors_departures_status_badge_class((string)($flight['status'] ?? '')),
                    'containers_total' => $containersTotal,
                    'packages_count' => connectors_departures_format_value($flight['packages_count'] ?? null, 0),
                    'total_weight' => connectors_departures_format_value($flight['total_weight'] ?? null),
                    'containers_sync_status' => trim((string)($flight['containers_sync_status'] ?? 'pending')),
                    'containers_synced_at' => connectors_departures_format_datetime($flight['containers_synced_at'] ?? null),
                    'containers_sync_error' => trim((string)($flight['containers_sync_error'] ?? '')),
                    'updated_at' => connectors_departures_format_datetime($updatedAtRaw),
                    'flight_time' => trim((string)($flight['flight_time'] ?? '')),
                    'closed_at' => connectors_departures_format_datetime($closedAtRaw),
...
switch ($dispatchAction) {
    ...
    case 'departures_flights':
        ...
        $connectors = [];
        if ($forwarderId > 0) {
            $connector = connectors_fetch_one($dbcnx, $forwarderId);
            if ($connector) {
            ...
            if ($res = $dbcnx->query($sql)) {
                while ($row = $res->fetch_assoc()) {
                    $connectors[] = connectors_try_migrate_operations_json($row);
                }
                $res->free();
            }
        }
        ...
        foreach ($connectors as $connector) {
            ...
            $departureRows = array_merge(
                $departureRows,
                connectors_departures_fetch_rows($dbcnx, $connector, $statusFilter)
            );
        }
