...
function connectors_resolve_report_operation_id(array $runtimeOperations): ?string
{
    ...
    if (
        isset($runtimeOperations['report'])
        && is_array($runtimeOperations['report'])
        && !empty($runtimeOperations['report']['enabled'])
    ) {
        return 'report';
    }
    foreach ($runtimeOperations as $operationId => $operation) {
...
function connectors_report_column_exists(mysqli $dbcnx, string $tableName, string $column): bool
{
    ...
    if (!$result instanceof mysqli_result) {
        throw new RuntimeException('DB error (check report column): ' . $dbcnx->error);
    }
...
function connectors_report_index_exists(mysqli $dbcnx, string $tableName, string $indexName): bool
{
    $safeTable = str_replace('`', '``', $tableName);
    $safeIndex = $dbcnx->real_escape_string($indexName);
    $sql = "SHOW INDEX FROM `{$safeTable}` WHERE Key_name = '{$safeIndex}'";
    $result = $dbcnx->query($sql);
    if (!$result instanceof mysqli_result) {
        throw new RuntimeException('DB error (check report index): ' . $dbcnx->error);
    }
...
function connectors_report_has_payload_duplicates(mysqli $dbcnx, string $tableName): bool
{
    $safeTable = '`' . str_replace('`', '``', $tableName) . '`';
    $sql = "SELECT 1 FROM {$safeTable} GROUP BY connector_id, payload_sha256 HAVING COUNT(*) > 1 LIMIT 1";
    $result = $dbcnx->query($sql);
    if (!$result instanceof mysqli_result) {
        throw new RuntimeException('DB error (check report duplicates): ' . $dbcnx->error);
    }
...
"
        . " payload_sha256 CHAR(64) GENERATED ALWAYS AS (SHA2(payload_json, 256)) STORED,
"
...
"
        . " KEY idx_report_payload_sha256 (connector_id, payload_sha256),
"
        . " UNIQUE KEY uq_report_payload_sha256 (connector_id, payload_sha256)
"
        . ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci";
    ...
    if (!connectors_report_column_exists($dbcnx, $tableName, 'payload_sha256')) {
        $dbcnx->query("ALTER TABLE {$safeTable} ADD COLUMN payload_sha256 CHAR(64) GENERATED ALWAYS AS (SHA2(payload_json, 256)) STORED AFTER payload_json");
    }
    if (!connectors_report_column_exists($dbcnx, $tableName, 'seen_count')) {
        $dbcnx->query("ALTER TABLE {$safeTable} ADD COLUMN seen_count INT UNSIGNED NOT NULL DEFAULT 1 AFTER payload_sha256");
    }
    if (!connectors_report_column_exists($dbcnx, $tableName, 'last_seen_at')) {
        $dbcnx->query("ALTER TABLE {$safeTable} ADD COLUMN last_seen_at DATETIME NULL AFTER seen_count");
    }
    if (!connectors_report_column_exists($dbcnx, $tableName, 'last_source_file')) {
        $dbcnx->query("ALTER TABLE {$safeTable} ADD COLUMN last_source_file VARCHAR(255) NULL AFTER last_seen_at");
    }

    if (!connectors_report_index_exists($dbcnx, $tableName, 'idx_report_payload_sha256')) {
        $dbcnx->query("ALTER TABLE {$safeTable} ADD INDEX idx_report_payload_sha256 (connector_id, payload_sha256)");
    }

    if (!connectors_report_index_exists($dbcnx, $tableName, 'uq_report_payload_sha256')) {
        if (!connectors_report_has_payload_duplicates($dbcnx, $tableName)) {
            $dbcnx->query("ALTER TABLE {$safeTable} ADD UNIQUE KEY uq_report_payload_sha256 (connector_id, payload_sha256)");
        } else {
            error_log('connector report table requires dedup before UNIQUE KEY uq_report_payload_sha256: ' . $tableName . '. Run clean_connector_report_table.php');
        }
    }
}

function connectors_report_insert_unique(
    mysqli $dbcnx,
    string $targetTable,
    int $connectorId,
    ?string $periodFrom,
...
{
    if (!preg_match('/^connector_report_[a-zA-Z0-9_]+$/', $targetTable)) {
        throw new InvalidArgumentException('Некорректное имя таблицы отчета');
    }

    $safeTable = '`' . str_replace('`', '``', $targetTable) . '`';
    $sql = 'INSERT INTO ' . $safeTable . ' (connector_id, period_from, period_to, payload_json, source_file, created_at, seen_count, last_seen_at, last_source_file) '
        . 'VALUES (?, ?, ?, ?, ?, NOW(), 1, NOW(), ?) '
        . 'ON DUPLICATE KEY UPDATE seen_count = seen_count + 1, last_seen_at = NOW(), last_source_file = VALUES(source_file)';

    $stmt = $dbcnx->prepare($sql);
    if (!$stmt) {
        throw new RuntimeException('DB error (prepare report unique insert): ' . $dbcnx->error);
    }

    $stmt->bind_param('isssss', $connectorId, $periodFrom, $periodTo, $payloadJson, $sourceFile, $sourceFile);
    if (!$stmt->execute()) {
        $err = $stmt->error;
        $stmt->close();
        throw new RuntimeException('DB error (execute report unique insert): ' . $err);
    }
...
function connectors_is_node_execution_enabled(): bool
{
    static $cached = null;
    if ($cached !== null) {
        return $cached;
    ...
    $raw = getenv('CONNECTORS_ALLOW_NODE');
...
function connectors_assert_node_execution_allowed(string $context = ''): void
{
    if (!connectors_is_node_execution_enabled()) {
        $suffix = $context !== '' ? (' (' . $context . ')') : '';
        throw new RuntimeException('Node execution отключен через CONNECTORS_ALLOW_NODE=0' . $suffix . '. Используйте PHP-операцию (kind=php_report или kind=script + interpreter=php).');
    }

    if (!connectors_is_node_runtime_available()) {
        $suffix = $context !== '' ? (' (' . $context . ')') : '';
        throw new RuntimeException('Node runtime недоступен' . $suffix . '. Используйте PHP-операцию (kind=php_report или kind=script + interpreter=php).');
    }
}
...
function connectors_download_report_file(array $connector, array $reportCfg, ?string $periodFrom, ?string $periodTo): array
{
    ...
    if (($reportCfg['download_mode'] ?? 'browser') === 'browser') {
        ...
        connectors_assert_node_execution_allowed('download_mode=browser');
...
function connectors_import_csv_into_report_table(mysqli $dbcnx, string $tableName, string $filePath, int $connectorId, ?string $periodFrom, ?string $periodTo, array $fieldMapping): int
{
    ...
    while (($row = fgetcsv($fh)) !== false) {
        ...
        $inserted = connectors_report_insert_unique($dbcnx, $tableName, (int)$connectorId, $periodFrom, $periodTo, $payloadJson, $sourceFile);
        if ($inserted) {
            $count++;
        } else {
...
function connectors_run_submission_test(array $connector, array $submissionCfg): array
{
    ...
    connectors_assert_node_execution_allowed('submission_test');
    $cmd = 'node ' . escapeshellarg($scriptPath) . ' ' . escapeshellarg((string)json_encode($payload, JSON_UNESCAPED_UNICODE));
    $output = connectors_run_shell_command_with_timeout($cmd . ' 2>&1', 90);
    $decoded = json_decode(trim((string)$output), true);
    if (!is_array($decoded)) {
...
function connectors_execute_script_operation(array $operation, array $connector = [], ?string $periodFrom = null, ?string $periodTo = null): array
{
    ...
    if ($interpreter === 'node') {
        connectors_assert_node_execution_allowed('kind=script');
    }
    ...
    $isContainerFlightScript = in_array($scriptBasename, [
        'run_add_container_to_flight.php',
        'run_del_container_from_flight.php',
        'run_sync_flight_containers.php',
    ], true);

    $resolvedContainersTable = trim((string)($config['containers_table'] ?? ''));
    if (
        $isContainerFlightScript
        && $resolvedContainersTable === ''
        && $resolvedTargetTable !== ''
        && preg_match('/_containers$/i', $resolvedTargetTable)
    ) {
        $resolvedContainersTable = $resolvedTargetTable;
        $resolvedTargetTable = preg_replace('/_containers$/i', '', $resolvedTargetTable) ?? $resolvedTargetTable;
    }
    if ($resolvedContainersTable === '' && $resolvedTargetTable !== '') {
        $resolvedContainersTable = $resolvedTargetTable . '_containers';
    }
    ...
    if (isset($config['args']) && is_array($config['args'])) {
        foreach ($config['args'] as $arg) {
            ...
            if (preg_match('/^--containers(?:-|_)table=/i', trim($expandedArg))) {
                $hasContainersTableArg = true;
            }
        }
    }
    ...
    if (
        !$hasContainersTableArg
        && $resolvedContainersTable !== ''
        && $isContainerFlightScript
    ) {
        $forcedContainersArg = '--containers-table=' . $resolvedContainersTable;
        $args[] = $forcedContainersArg;
        $argsMasked[] = connectors_mask_script_arg($forcedContainersArg);
    }
...
switch ($dispatchAction) {
    ...
    case 'form_connector_operations':
        ...
        $nodeRuntimeAvailable = connectors_is_node_runtime_available() && connectors_is_node_execution_enabled();
