...
function departures_extract_runtime_operations(array $connector): array
{
    ...
    if (isset($decoded['operations']) && is_array($decoded['operations'])) {
        ...
        foreach ($decoded['operations'] as $operation) {
            ...
            if (!isset($config['target_table']) && isset($operation['target_table'])) {
                $config['target_table'] = $operation['target_table'];
            }
            if (!isset($config['script_path']) && isset($operation['script_path'])) {
                $config['script_path'] = $operation['script_path'];
            }
...
function departures_resolve_target_table_from_operations(array $connector): string
{
    $runtimeOperations = departures_extract_runtime_operations($connector);
    foreach ($runtimeOperations as $operationId => $operation) {
        ...
        $config = isset($operation['config']) && is_array($operation['config']) ? $operation['config'] : [];
        $targetTable = trim((string)($config['target_table'] ?? ''));
        if ($targetTable === '') {
            continue;
        }
        ...
        if (
            in_array((string)$operationId, ['flight_list_php', 'add_container_to_flight_php', 'delete_container_php'], true)
            || in_array($scriptBasename, ['run_flight_list.php', 'run_add_container_to_flight.php', 'run_del_container_from_flight.php', 'run_sync_flight_containers.php'], true)
        ) {
            if (preg_match('/_containers$/i', $targetTable)) {
                $targetTable = preg_replace('/_containers$/i', '', $targetTable) ?? $targetTable;
            }
            return connectors_subrunner_sanitize_table_name($targetTable);
        }
    }
...
function departures_load_containers_from_table(mysqli $dbcnx, string $flightTableName, int $connectorId, int $flightRecordId, string $flightExternalId): array
{
    ...
    $rawRows = [];
    $containerLookupKeys = [];
    $loadRows = static function (string $sql, string $types, array $params) use ($dbcnx, &$rawRows, &$containerLookupKeys): bool {
        $stmt = $dbcnx->prepare($sql);
        if (!$stmt) {
    ...
    if ($flightRecordId > 0) {
        $byRecordIdSql = "SELECT {$selectSql}
                            FROM {$safeContainersTable}
                           WHERE connector_id = ? AND flight_record_id = ? AND is_active = 1
                           ORDER BY updated_at DESC, id DESC";
        if (!$loadRows($byRecordIdSql, 'ii', [$connectorId, $flightRecordId])) {
            return [];
        }
    }


    if ($rawRows === [] && $flightExternalId !== '') {
        $byExternalIdSql = "SELECT {$selectSql}
                              FROM {$safeContainersTable}
                             WHERE connector_id = ? AND flight_external_id = ? AND is_active = 1
                             ORDER BY updated_at DESC, id DESC";
        if (!$loadRows($byExternalIdSql, 'is', [$connectorId, $flightExternalId])) {
            return [];
        }
    }
...
function departures_detect_flight_table_name(mysqli $dbcnx, array $connector, int $flightRecordId, string $flightId): string
{
    $connectorId = (int)($connector['id'] ?? 0);
    if ($connectorId <= 0) {
        return '';
    ...
    foreach (departures_resolve_table_names($connector) as $tableName) {
        if ($tableName === '' || !departures_table_exists($dbcnx, $tableName)) {
            continue;
        }

        $safeTable = '`' . str_replace('`', '``', $tableName) . '`';
        if ($flightRecordId > 0) {
            $stmt = $dbcnx->prepare("SELECT id FROM {$safeTable} WHERE connector_id = ? AND id = ? LIMIT 1");
            if ($stmt) {
        ...
        if ($flightId !== '') {
            $stmt = $dbcnx->prepare("SELECT id FROM {$safeTable} WHERE connector_id = ? AND external_id = ? LIMIT 1");
            if (!$stmt) {
                continue;
            }
...
function departures_sync_flight_containers_from_forwarder(array $connector, string $flightId, string $flightTableName = ''): array
{
    $connectorId = (int)($connector['id'] ?? 0);
    ...
    if ($flightTableName === '') {
        $flightTableName = departures_resolve_target_table_from_operations($connector);
    }
    if ($flightTableName === '') {
        foreach (departures_resolve_table_names($connector) as $candidateTableName) {
            if ($candidateTableName !== '') {
                $flightTableName = $candidateTableName;
                break;
            }
        }
    }
    $containersTableName = $flightTableName !== ''
        ? connectors_subrunner_resolve_flight_containers_table_name($flightTableName, [])
        : '';
    ...
    if ($flightTableName !== '') {
        $cmdParts[] = '--target-table=' . $flightTableName;
    }
    if ($containersTableName !== '') {
        $cmdParts[] = '--containers-table=' . $containersTableName;
    }
    ...
    case 'departures_container_action':
        ...
        try {
            if ($operation === 'sync_forwarder') {
                $flightTableName = departures_detect_flight_table_name($dbcnx, $connector, $flightRecordId, $flightId);
                $syncResult = departures_sync_flight_containers_from_forwarder($connector, $flightId, $flightTableName);
                $compareRefresh = null;
