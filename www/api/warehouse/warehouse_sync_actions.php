...
if (!function_exists('warehouse_sync_queue_print_preview')) {
    ...
    function warehouse_sync_escape_pdf_text(string $text): string
    {
    ...
    {
        $plain = preg_replace('#<(br|/p|/div|/li|/h[1-6])\b[^>]*>#iu', "\n", $previewHtml) ?? $previewHtml;
        ...
        $pageWidth = max(120.0, ($widthCm / 2.54) * 72.0);
        $pageHeight = max(120.0, ($heightCm / 2.54) * 72.0);
        $startY = max(30.0, $pageHeight - 30.0);
        $textCommands = ['BT', '/F1 11 Tf', '20 ' . rtrim(rtrim(number_format($startY, 2, '.', ''), '0'), '.') . ' Td'];
        $lineIndex = 0;
        foreach ($lines as $line) {
            $escaped = warehouse_sync_escape_pdf_text((string)$line);
            if ($lineIndex > 0) {
                $textCommands[] = '0 -18 Td';
            }
            $textCommands[] = '(' . $escaped . ') Tj';
            $lineIndex++;
        }
        $textCommands[] = 'ET';
        $stream = implode("\n", $textCommands) . "\n";
        ...
        $rotateValue = in_array($rotateDegrees, [0, 90, 180, 270], true) ? $rotateDegrees : 0;
        $rotateChunk = $rotateValue !== 0 ? " /Rotate {$rotateValue}" : '';
        $mediaBox = '0 0 '
            . rtrim(rtrim(number_format($pageWidth, 2, '.', ''), '0'), '.')
            . ' '
            . rtrim(rtrim(number_format($pageHeight, 2, '.', ''), '0'), '.');

        $objects = [
            "1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj\n",
            "2 0 obj\n<< /Type /Pages /Kids [3 0 R] /Count 1 >>\nendobj\n",
            "3 0 obj\n<< /Type /Page /Parent 2 0 R /MediaBox [{$mediaBox}]{$rotateChunk} /Contents 4 0 R /Resources << /Font << /F1 5 0 R >> >> >>\nendobj\n",
            "4 0 obj\n<< /Length {$streamLen} >>\nstream\n{$stream}endstream\nendobj\n",
    ...
    ): array
    {
        ...
        if ($html === '') {
            return ['ok' => false, 'error' => 'preview html is empty', 'pdf_base64' => ''];
        }
        ...
        $tmpPdf = tempnam($tmpDir, 'ws_preview_pdf_');
    ...
    ): array
    {
        ...
        $renderWidthCm = ((float)$printProfile['render_width_mm']) / 10;
        $renderHeightCm = ((float)$printProfile['render_height_mm']) / 10;
        $paperWidthCm = ((float)$printProfile['paper_width_mm']) / 10;
        $paperHeightCm = ((float)$printProfile['paper_height_mm']) / 10;
        $renderRotate = (int)$printProfile['render_rotate'];
        ...
        if ($preferRaster) {
            ...
            $intermediatePngBase64 = (string)($renderPng['png_base64'] ?? '');
            $renderEngine = 'html-to-png';
            $renderMessage = trim((string)($renderPng['error'] ?? ''));
            if ($intermediatePngBase64 !== '') {
                $composed = warehouse_sync_compose_physical_png_base64($intermediatePngBase64, $printProfile);
                $composeDiagnostics = is_array($composed['diagnostics'] ?? null) ? $composed['diagnostics'] : [];
                $labelBase64 = (string)($composed['png_base64'] ?? '');
                $renderEngine = 'html-to-png+physical-compose';
                $renderMessage = trim($renderMessage . ' ' . (string)($composed['error'] ?? ''));
        ...
        $job = [
            ...
            'print_paper_width_mm' => $printProfile['paper_width_mm'],
            'print_paper_height_mm' => $printProfile['paper_height_mm'],
        ];
...
if (!function_exists('warehouse_sync_resolve_label_template_code')) {
    function warehouse_sync_resolve_label_template_code(mysqli $dbcnx, array $connector): array
    {
        ...
        $labelWidthCm = (float)($row['label_width_cm'] ?? 10.0);
        $labelHeightCm = (float)($row['label_height_cm'] ?? 15.0);
        $printRotate = (int)($row['print_rotate'] ?? 0);
...
if (!function_exists('warehouse_sync_label_render_defaults')) {
    ...
    function warehouse_sync_build_print_profile_from_template(array $template, array $post = []): array
    {
        $source = array_merge($template, $post);
        $profile = warehouse_sync_label_render_profile_from_array($source);

        return [
            'paper_width_mm' => (float)$profile['print_paper_width_mm'],
            'paper_height_mm' => (float)$profile['print_paper_height_mm'],
            'render_width_mm' => (float)$profile['render_width_mm'],
            'render_height_mm' => (float)$profile['render_height_mm'],
            'render_rotate' => (int)$profile['render_rotate'],
            'layout_mode' => (string)$profile['render_layout_mode'],
            'css_override' => (bool)$profile['render_css_override'],
            'fit_mode' => (string)$profile['render_fit_mode'],
            'scale_percent' => (float)$profile['render_scale_percent'],
            'offset_x_mm' => (float)$profile['render_offset_x_mm'],
            'offset_y_mm' => (float)$profile['render_offset_y_mm'],
        ];
    }
}
...
if (!function_exists('warehouse_sync_compose_physical_png_base64')) {
    ...
    function warehouse_sync_compose_physical_png_base64(string $intermediatePngBase64, array $printProfile, int $dpi = 203): array
    {
        ...
        if (!is_string($binary) || $binary === '') {
            return ['ok' => false, 'error' => 'intermediate png is empty', 'png_base64' => '', 'diagnostics' => []];
        }
        if (!function_exists('imagecreatefromstring')) {
            return ['ok' => false, 'error' => 'GD extension is required to compose physical raster', 'png_base64' => '', 'diagnostics' => []];
        }

        $src = @imagecreatefromstring($binary);
        if (!$src) {
            return ['ok' => false, 'error' => 'failed to decode intermediate png', 'png_base64' => '', 'diagnostics' => []];
        }

        $rotate = (int)($printProfile['render_rotate'] ?? 0);
        if (in_array($rotate, [90, 180, 270], true)) {
            $src = imagerotate($src, 360 - $rotate, 0);
        }
        ...
        $srcW = imagesx($src);
        $srcH = imagesy($src);
        $paperWmm = max(1.0, (float)($printProfile['paper_width_mm'] ?? 100.0));
        $paperHmm = max(1.0, (float)($printProfile['paper_height_mm'] ?? 150.0));
        $dstW = max(1, (int)round(($paperWmm / 25.4) * $dpi));
        $dstH = max(1, (int)round(($paperHmm / 25.4) * $dpi));

        $dst = imagecreatetruecolor($dstW, $dstH);
        $white = imagecolorallocate($dst, 255, 255, 255);
        imagefilledrectangle($dst, 0, 0, $dstW, $dstH, $white);

        $fitMode = (string)($printProfile['fit_mode'] ?? 'contain');
        $scale = max(1.0, (float)($printProfile['scale_percent'] ?? 100.0)) / 100.0;
        $ratioX = $dstW / max(1, $srcW);
        $ratioY = $dstH / max(1, $srcH);
        $fitRatio = $fitMode === 'cover' ? max($ratioX, $ratioY) : ($fitMode === 'none' ? 1.0 : min($ratioX, $ratioY));
        $copyW = max(1, (int)round($srcW * $fitRatio * $scale));
        $copyH = max(1, (int)round($srcH * $fitRatio * $scale));
        $offsetX = (int)round(((float)($printProfile['offset_x_mm'] ?? 0.0) / 25.4) * $dpi);
        $offsetY = (int)round(((float)($printProfile['offset_y_mm'] ?? 0.0) / 25.4) * $dpi);
        $dstX = (int)round(($dstW - $copyW) / 2) + $offsetX;
        $dstY = (int)round(($dstH - $copyH) / 2) + $offsetY;

        imagecopyresampled($dst, $src, $dstX, $dstY, 0, 0, $copyW, $copyH, $srcW, $srcH);
        ob_start();
        imagepng($dst);
        ...
        $paperAspect = $paperWmm / $paperHmm;
        $finalAspect = $dstW / $dstH;
        $warning = abs($paperAspect - $finalAspect) > 0.01 ? 'final raster aspect does not match CUPS media' : '';

        return [
            'ok' => $png !== '',
            'error' => $png !== '' ? '' : 'failed to encode final png',
            'png_base64' => base64_encode($png),
            'diagnostics' => [
                'intermediate_png_width_px' => $srcW,
                'intermediate_png_height_px' => $srcH,
                'final_png_width_px' => $dstW,
                'final_png_height_px' => $dstH,
                'final_raster_aspect_warning' => $warning,
            ],
        ];
    }
}
...
if (!function_exists('warehouse_sync_ensure_label_templates_table')) {
    function warehouse_sync_ensure_label_templates_table(mysqli $dbcnx): bool
    {
        $ensureColumns = static function () use ($dbcnx): bool {
            ...
            if (empty($columns['label_width_cm'])) {
                $missing[] = "ADD COLUMN label_width_cm DECIMAL(5,2) NOT NULL DEFAULT 10.00 AFTER priority";
            }
            if (empty($columns['label_height_cm'])) {
                $missing[] = "ADD COLUMN label_height_cm DECIMAL(5,2) NOT NULL DEFAULT 15.00 AFTER label_width_cm";
            }
...
if (!function_exists('warehouse_sync_first_non_empty_value')) {
    function warehouse_sync_first_non_empty_value(array ...$sources): string
    {
...
if (!function_exists('warehouse_sync_decode_addons_json')) {
    function warehouse_sync_decode_addons_json($raw): array
    {
...
if (!function_exists('warehouse_sync_fetch_forwarder_report_item')) {
    function warehouse_sync_fetch_forwarder_report_item(mysqli $dbcnx, int $connectorId, string $trackingNo): array
    {
        if ($connectorId <= 0 || trim($trackingNo) === '' || !warehouse_sync_table_exists($dbcnx, 'forwarder_report_items')) {
            return [];
        }
        $columns = warehouse_sync_table_columns($dbcnx, 'forwarder_report_items');
        if (!in_array('connector_id', $columns, true) || !in_array('tracking_no', $columns, true)) {
            return [];
        }
        $stmt = $dbcnx->prepare("SELECT * FROM forwarder_report_items WHERE connector_id = ? AND tracking_no = ? LIMIT 1");
        if (!$stmt) {
            return [];
        }
        $stmt->bind_param('is', $connectorId, $trackingNo);
        $stmt->execute();
        $row = $stmt->get_result()?->fetch_assoc();
        $stmt->close();
...
if (!function_exists('warehouse_sync_build_out_label_vars')) {
    function warehouse_sync_build_out_label_vars(mysqli $dbcnx, array $item, array $connector, array $context): array
    {
        $trackingNo = warehouse_sync_first_non_empty_value([
            $context['tracking_no'] ?? '', $item['tracking_no'] ?? '', $item['stock_tracking_no'] ?? '', $item['tuid'] ?? '', $item['stock_tuid'] ?? '',
        ]);
        $addons = warehouse_sync_decode_addons_json($item['addons_json'] ?? '');
        $report = warehouse_sync_fetch_forwarder_report_item($dbcnx, (int)($connector['id'] ?? 0), $trackingNo);
        $pick = static function (array $keys) use ($item, $addons, $report): string {
            $values = [];
            foreach ($keys as $key) {
                $values[] = $item[$key] ?? '';
                $values[] = $addons[$key] ?? '';
            ...
            return warehouse_sync_first_non_empty_value($values);
        };
        $clientName = warehouse_sync_first_non_empty_value([
            $item['receiver_name'] ?? '', $item['stock_receiver_name'] ?? '', $item['resolved_receiver_company'] ?? '', $item['receiver_company'] ?? '', $item['stock_receiver_company'] ?? '',
            $addons['client_name'] ?? '', $report['client_name'] ?? '', $report['receiver_name'] ?? '', $report['receiver_company'] ?? '',
        ]);
        $countryCode = strtoupper(warehouse_sync_first_non_empty_value([
            $item['resolved_receiver_country_code'] ?? '', $item['receiver_country_code'] ?? '', $item['stock_receiver_country_code'] ?? '', $addons['receiver_country_code'] ?? '', $addons['country_dest'] ?? '', $report['receiver_country_code'] ?? '', $report['country_dest'] ?? '',
        ]));
        $vars = [
            '{{track}}' => $trackingNo,
            '{{client_name}}' => $clientName,
            '{{client}}' => $clientName,
            '{{client_address}}' => warehouse_sync_first_non_empty_value([$item['resolved_receiver_address'] ?? '', $item['receiver_address'] ?? '', $item['stock_receiver_address'] ?? '', $addons['client_address'] ?? '', $report['client_address'] ?? '', $report['receiver_address'] ?? '']),
            '{{receiver_country_code}}' => $countryCode,
            '{{country_dest}}' => warehouse_sync_first_non_empty_value([$countryCode, $addons['country_dest'] ?? '', $report['country_dest'] ?? '']),
            '{{forward_name}}' => trim((string)($connector['name'] ?? '')),
            '{{flight_name}}' => trim((string)($context['flight_display'] ?? '')),
            '{{shipment_cell}}' => trim((string)($context['shipment_cell'] ?? '')),
            '{{container_name}}' => trim((string)($context['container_display'] ?? '')),
            '{{weight}}' => $pick(['weight_kg', 'weight']),
            '{{volume_weight}}' => $pick(['volume_weight']),
            '{{description}}' => $pick(['description', 'goods_description', 'category']),
            '{{amount}}' => $pick(['amount']),
            '{{invoice_usd}}' => $pick(['invoice_usd']),
            '{{total_invoice_price}}' => $pick(['total_invoice_price']),
            '{{declared_value}}' => $pick(['declared_value', 'price']),
            '{{invoice_price}}' => $pick(['invoice_price', 'price']),
            '{{shipping_price}}' => $pick(['shipping_price']),
            '{{seller}}' => $pick(['seller']),
            '{{client_code}}' => $pick(['client_code']),
            '{{client_id}}' => $pick(['client_id']),
            '{{consignee_phone}}' => $pick(['consignee_phone', 'phone']),
            '{{container_position}}' => trim((string)($context['container_position'] ?? '')),
            '{{flight_record_id}}' => trim((string)($context['flight_record_id'] ?? '')),
        ];
        foreach (['track', 'client_name', 'client_address', 'receiver_country_code', 'country_dest', 'forward_name', 'flight_name', 'weight', 'description', 'amount', 'invoice_price'] as $key) {
...
if (!function_exists('warehouse_sync_label_vars_have_test_values')) {
    function warehouse_sync_label_vars_have_test_values(array $labelVars): bool
    {
        $haystack = implode("\n", array_map(static fn($value): string => is_scalar($value) ? (string)$value : '', $labelVars));
        foreach (['Test Client', 'Baku, Test street', 'Test goods description', 'TEST-TRACK', 'CLI001', 'INT-00001'] as $needle) {
...
if (!function_exists('zpl_escape')) {
    function zpl_escape($text): string
    {
        return warehouse_sync_zpl_text((string)$text, 0);
    }
}
...
if (!function_exists('zpl_text')) {
    function zpl_text($x, $y, $fontH, $fontW, $text, $maxChars = null): string
    {
        $value = zpl_escape((string)$text);
...
if (!function_exists('zpl_line_h')) {
    function zpl_line_h($x, $y, $w, $thickness = 1): string
    {
        return zpl_box((int)$x, (int)$y, (int)$w, (int)$thickness, (int)$thickness);
    }
}

if (!function_exists('zpl_line_v')) {
    function zpl_line_v($x, $y, $h, $thickness = 1): string
    {
        return zpl_box((int)$x, (int)$y, (int)$thickness, (int)$h, (int)$thickness);
    }
}

if (!function_exists('zpl_barcode_code128')) {
    function zpl_barcode_code128($x, $y, $w, $h, $value): string
    {
...
if (!function_exists('warehouse_sync_zpl_wrap')) {
    function warehouse_sync_zpl_wrap(string $text, int $charsPerLine, int $maxLines): array
    {
        ...
        if (function_exists('mb_strlen') && function_exists('mb_substr')) {
            ...
            foreach ($words as $word) {
                ...
                if (mb_strlen($candidate, 'UTF-8') <= $charsPerLine) {
                    $line = $candidate;
                    continue;
                }
                if ($line !== '') {
                ...
                $line = mb_substr($word, 0, $charsPerLine, 'UTF-8');
        ...
        return array_slice(str_split($text, $charsPerLine), 0, $maxLines);
    }
}
...
if (!function_exists('warehouse_sync_render_waybill_zpl')) {
    function warehouse_sync_render_waybill_zpl(array $vars, array $profile = []): string
    {
        ...
        $forwardName = warehouse_sync_label_var($vars, 'forward_name', 'DEV COLIBRI');
        $countryDest = warehouse_sync_label_var($vars, 'country_dest', '');
        $flightDeparture = warehouse_sync_label_var($vars, 'flight_departure', 'HHN');
        $flightDestination = warehouse_sync_label_var($vars, 'flight_destination', 'GYD');
        $flightName = warehouse_sync_label_var($vars, 'flight_name', 'AZ001');
        ...
        $shipper = trim($clientName . ($clientCode !== '' ? ' / ' . $clientCode : ''));
        ...
        $z[] = zpl_text(225, 56, 22, 20, $clientCode !== '' ? $clientCode : $clientId, 24);
        $z[] = zpl_text(350, 25, 19, 18, 'Charge Collect', 18);
        $z[] = zpl_text(350, 55, 19, 18, 'Prepaid', 18);

        $z[] = zpl_line_h(0, 255, 528, 2);
        $z[] = zpl_text(16, 105, 20, 19, '2 From/Shipper', 18);
        $z[] = zpl_text(25, 140, 25, 23, $shipper, 32);
        foreach (warehouse_sync_zpl_wrap($address, 42, 2) as $i => $line) {
            $z[] = zpl_text(25, 176 + $i * 28, 20, 18, $line, 42);
        }
        $z[] = zpl_text(25, 225, 18, 17, 'Postcode:  Phone: ' . $phone, 44);

        $z[] = zpl_line_h(0, 465, 528, 2);
        $z[] = zpl_text(16, 270, 20, 19, '3 To Consignee', 18);
        $z[] = zpl_text(25, 305, 25, 23, $clientName, 34);
        $z[] = zpl_text(25, 336, 18, 17, 'Personal ID: ' . $clientId, 38);
        $z[] = zpl_text(25, 364, 18, 17, 'Consignee phone: ' . $phone, 42);
        $z[] = zpl_text(25, 394, 18, 17, 'Delivery Address:', 24);
        foreach (warehouse_sync_zpl_wrap($address, 46, 2) as $i => $line) {
            $z[] = zpl_text(185, 394 + $i * 25, 18, 16, $line, 46);
        }
        $z[] = zpl_barcode_code128(105, 485, 330, 78, $barcodeValue);
        $z[] = zpl_text(180, 575, 24, 22, $barcodeValue, 34);
        $z[] = zpl_text(25, 625, 18, 16, 'Postcode / Country: ' . $countryDest, 44);
        $z[] = zpl_text(25, 655, 18, 16, 'Contact person: ' . $clientName, 44);
        $z[] = zpl_text(25, 735, 16, 15, 'TRACK: ' . $track, 50);

        $z[] = zpl_text(560, 18, 54, 50, $forwardName !== '' ? $forwardName : 'DEV COLIBRI', 22);
        $z[] = zpl_box(1030, 15, 130, 70, 2);
        $z[] = zpl_text(1052, 31, 20, 18, 'CDN / QR', 12);
        $z[] = zpl_qr(1100, 24, $track);
        $z[] = zpl_text(560, 102, 38, 35, $route !== '' ? $route : 'HHN / GYD', 16);
        $z[] = zpl_text(820, 108, 28, 26, $flightName, 18);

        $z[] = zpl_line_h(528, 155, 672, 2);
        $z[] = zpl_text(545, 122, 20, 19, '4 Shipment details', 22);
        $cols = [528, 696, 864, 1032, 1200];
        foreach ($cols as $cx) { $z[] = zpl_line_v($cx, 155, 145, 1); }
        $z[] = zpl_line_h(528, 210, 672, 1);
        $z[] = zpl_line_h(528, 300, 672, 2);
        $z[] = zpl_text(545, 170, 17, 16, 'Packages', 14);
        $z[] = zpl_text(713, 170, 17, 16, 'Gross weight', 16);
        $z[] = zpl_text(881, 170, 17, 16, 'Volume weight', 16);
        $z[] = zpl_text(1049, 170, 17, 16, 'Shipping price', 18);
        $z[] = zpl_text(590, 232, 26, 24, '1', 8);
        $z[] = zpl_text(722, 232, 24, 22, $weight, 14);
        $z[] = zpl_text(890, 232, 24, 22, $volumeWeight, 14);
        $z[] = zpl_text(1050, 232, 24, 22, $amount, 18);

        $z[] = zpl_line_h(528, 360, 672, 2);
        $z[] = zpl_text(545, 318, 18, 17, 'Transportation mode', 24);
        $z[] = zpl_text(760, 315, 28, 26, 'By Air', 12);
        $z[] = zpl_text(950, 318, 18, 17, 'MAWB / flight #', 20);
        $z[] = zpl_text(1090, 315, 28, 26, $flightName, 18);

        $z[] = zpl_line_h(528, 430, 672, 2);
        $z[] = zpl_text(545, 382, 20, 19, '5 Full Description', 24);
        foreach (warehouse_sync_zpl_wrap($description, 66, 3) as $i => $line) {
            $z[] = zpl_text(545, 440 + $i * 30, 21, 19, $line, 66);
        }
        $z[] = zpl_line_h(528, 560, 672, 1);
        $z[] = zpl_line_v(752, 560, 95, 1);
        $z[] = zpl_line_v(976, 560, 95, 1);
        $z[] = zpl_line_h(528, 610, 672, 1);
        $z[] = zpl_text(545, 575, 17, 16, 'Category', 12);
        $z[] = zpl_text(769, 575, 17, 16, 'Declared value', 18);
        $z[] = zpl_text(993, 575, 17, 16, 'Total price', 16);
        $z[] = zpl_text(545, 625, 22, 20, $category, 26);
...
if ($action === 'warehouse_item_out_confirm_send') {
    ...
    foreach ([
        ...
        if (in_array($column, $stockColumns, true)) {
            $stockSelects[] = "wi.`{$column}` AS stock_{$column}";
            $stockSelects[] = "wi.`{$column}` AS {$column}";
        } else {
            $stockSelects[] = "'' AS stock_{$column}";
            $stockSelects[] = "'' AS {$column}";
        }
    }
    $stockSelectSql = implode(",\n               ", $stockSelects);
    $wiCellSql = in_array('cell_id', $stockColumns, true) ? 'wi.cell_id' : "''";
    $wiReceiverCompanySql = in_array('receiver_company', $stockColumns, true) ? 'wi.receiver_company' : "''";
    $wiReceiverCountrySql = in_array('receiver_country_code', $stockColumns, true) ? 'wi.receiver_country_code' : "''";
    $wiReceiverAddressSql = in_array('receiver_address', $stockColumns, true) ? 'wi.receiver_address' : "''";

    $sqlSelect = "
        SELECT wo.*, {$stockSelectSql}, {$wiCellSql} AS cell_id,
               COALESCE(NULLIF(wo.receiver_company, ''), {$wiReceiverCompanySql}) AS resolved_receiver_company,
               COALESCE(NULLIF(wo.receiver_country_code, ''), {$wiReceiverCountrySql}) AS resolved_receiver_country_code,
               COALESCE(NULLIF(wo.receiver_address, ''), {$wiReceiverAddressSql}) AS resolved_receiver_address
        FROM warehouse_item_out wo
        LEFT JOIN warehouse_item_stock wi ON wi.id = wo.stock_item_id
...
if ($action === 'save_connector_label_template') {
    ...
    if ($labelHeightCm < 2.0 || $labelHeightCm > 30.0) {
        $response = ['status' => 'error', 'message' => 'label_height_cm должен быть в диапазоне 2..30 см'];
        return;
    }
    if (!in_array($printRotate, [0, 90, 180, 270], true)) {
        $response = ['status' => 'error', 'message' => 'print_rotate должен быть одним из: 0, 90, 180, 270'];
        return;
    }
    $connector = warehouse_sync_fetch_connector_by_id($dbcnx, $connectorId);
    if (!$connector) {
...
if ($action === 'warehouse_sync_single_check') {
    auth_require_login();
    ...
    try {
        ...
        $tracking = trim((string)($item['tracking_no'] ?? ''));
        if ($tracking === '') {
            $tracking = trim((string)($item['tuid'] ?? ''));
        }
        if ($tracking === '') {
            throw new RuntimeException('У посылки отсутствует tracking_no/tuid для проверки');
        }

        $checkArgs = [
            'base-url' => trim((string)($connector['base_url'] ?? '')),
            'login' => trim((string)($connector['auth_username'] ?? '')),
            'password' => trim((string)($connector['auth_password'] ?? '')),
            'track' => $tracking,
            'connector-id' => (string)((int)($connector['id'] ?? 0)),
        ];
        $checkResult = warehouse_sync_exec_forwarder_cli_script('run_report_single.php', $checkArgs);
        $checkMode = 'full_credentials';

        $checkStatus = strtoupper(trim((string)($checkResult['status'] ?? 'TEMP_ERROR')));
        $isOk = in_array($checkStatus, ['ACCEPTED', 'NOT_FOUND', 'AUTH_OK', 'AUTH_SKIPPED'], true);
        $statusMessage = sprintf(
            'Single check %s (mode: %s, track: %s, package_exist: %s, matched_track_found: %s)',
            $checkStatus,
            $checkMode,
            $tracking,
            !empty($checkResult['package_exist']) ? 'yes' : 'no',
            !empty($checkResult['matched_track_found']) ? 'yes' : 'no'
        );
        ...
        warehouse_sync_trace_log($dbcnx, [
            ...
            'connector_id' => (int)($connector['id'] ?? 0),
            'job_id' => max(0, (int)($_POST['job_id'] ?? 0)),
            'stage' => 'forwarder_single_check',
            'decision' => $isOk ? 'success' : 'error',
            'reason' => $statusMessage,
            'snapshot_json' => $snapshotJson === false ? '' : $snapshotJson,
        ]);

        warehouse_sync_audit_log($dbcnx, [
            'item_id' => $itemId,
            'tracking_no' => $tracking,
            'forwarder' => trim((string)($item['receiver_company'] ?? '')),
            'country_code' => trim((string)($item['receiver_country_code'] ?? '')),
            'status' => $isOk ? 'success' : 'error',
            'message' => $statusMessage,
            'response_json' => $snapshotJson === false ? '' : $snapshotJson,
...
if ($action === 'warehouse_sync_reconcile') {
    ...
    audit_log($userId, 'WAREHOUSE_SYNC_RECONCILE', 'warehouse_item_out', 0, 'Reconcile статусов warehouse_item_out выполнен', [
        'limit' => $limit,
        'backfill_inserted' => (int)($backfill['inserted'] ?? 0),
        'backfill_updated' => (int)($backfill['updated'] ?? 0),
        'backfill_processed' => (int)($backfill['processed'] ?? 0),
