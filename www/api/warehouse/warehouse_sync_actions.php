...
if (!function_exists('warehouse_sync_queue_print_preview')) {
    ...
    ): array
    {
        ...
        $fitScript = sprintf(
            ...
            . 'var scaleContain=Math.min(pageW/rotW,pageH/rotH);'
            . 'var scaleCover=Math.max(pageW/rotW,pageH/rotH);'
            . 'var useCover=(rotate===90||rotate===270);'
            . 'var scale=useCover?scaleCover:scaleContain;'
            . 'if(!isFinite(scale)||scale<=0){scale=1;}'
            . 'if(rotate===90){tx=pageW;}'
            . 'else if(rotate===180){tx=pageW;ty=pageH;}'
            . 'else if(rotate===270){ty=pageH;}'
            . 'root.style.transform="translate("+tx+"px,"+ty+"px) rotate("+rotate+"deg) scale("+scaleX+","+scaleY+")";'
...
if (!function_exists('warehouse_sync_label_template_preview_html')) {
    ...
    ): string
    {
        $sample = [
            ...
            '{{weight}}' => '1.000',
            '{{amount}}' => '10.00 USD',
            '{{country_dest}}' => 'Azerbaijan',
            '{{forward_name}}' => 'Colibri',
