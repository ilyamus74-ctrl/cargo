...
$size=filesize($path); $start=0; $end=$size-1; $name=str_replace(array("\r","\n",'"'), '_', (string)$file['original_name']);
header('Content-Type: '.$file['mime_type']); header('X-Content-Type-Options: nosniff');
if($file['file_type']==='document'){
    $disp=($file['mime_type']==='application/pdf' && ($_GET['disposition'] ?? '')==='inline') ? 'inline' : 'attachment';
