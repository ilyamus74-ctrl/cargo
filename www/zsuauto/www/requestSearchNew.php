...
//    }
//
//   $preg_list="SELECT * FROM `zs_announce_auto_uk` WHERE  `active_announce` != 'S'AND DATE(`date_in_announce`) > (NOW() - INTERVAL 3 DAY) ".$addon." ".$addon2." ".$addon3." ".$addon4." ".$addon5." ORDER BY `id` ASC LIMIT 8";
   $preg_list="SELECT * FROM `zs_announce_auto_uk` WHERE  `active_announce` != 'S'AND DATE(`date_in_announce`) > (NOW() - INTERVAL 3 DAY) ".$addon." ".$addon2." ".$addon3." ".$addon4." ".$addon5." ORDER BY `id` DESC";
    $preg_list_count ="SELECT COUNT(`id`) FROM `zs_announce_auto_uk` WHERE  `active_announce` != 'S' AND DATE(`date_in_announce`) > (NOW() - INTERVAL 9 DAY) ".$addon." ".$addon2." ".$addon3." ".$addon4." ".$addon5."  ORDER BY `id`DESC";
