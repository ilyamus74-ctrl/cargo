...
$_name_news_table="wh_news_auto";
$_name_anonce_table="wh_anonce_automobile";
function ccurl($url)
    {
    //echo "$url";
    $curl = curl_init();
...
$list_news=mysql_query("SELECT `name`,`main_url`,`url`,`preg_one`,`preg_link`,`preg_name`,`preg_anonce`,`preg_img`,`preg_text` FROM `$_name_news_table`  ORDER BY `date_in` ASC ") or die ("Invalid:" . mysql_error());
	while((list($name,$main_url,$url,$preg_one,$preg_link,$preg_name,$preg_anonce,$preg_img,$preg_text_full)=mysql_fetch_row($list_news)))
	{
...
$in_news=mysql_query("INSERT INTO `$_name_anonce_table` (`name_news`,`url_news`,`text_news`,`text_full_news`,`img_news`,`date_in_news`,`name_r`)
				 VALUES ('$name_n[1]','$link_n[1]','$anonce_n[1]','$item_text_full__','/img/news/$new_file$ext_file',now(),'$name')") or die("Invalid query: " . mysql_error());
