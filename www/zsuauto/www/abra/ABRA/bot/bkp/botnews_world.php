...
$list_news=mysql_query("SELECT `name`,`main_url`,`url`,`preg_one`,`preg_link`,`preg_name`,`preg_anonce`,`preg_img` FROM `wh_news_world`  ORDER BY `date_in` ASC ") or die ("Invalid:" . mysql_error());
	...
	if(!empty($url))
		...
		foreach($ccc[0] as $key=>$item)
		    ...
		    //print_r($txt_n[1]);
		    //print_r($img_n[1]);
		    $test_b=mysql_query("SELECT `name_news` FROM `wh_anonce_in_world` WHERE `url_news`='$link_n[1]' LIMIT 1 ") or die ("Invalid:" . mysql_error());
...
$in_news=mysql_query("INSERT INTO `wh_anonce_in_world` (`name_news`,`url_news`,`text_news`,`img_news`,`date_in_news`,`name_r`)
...
//echo "$llaset";
//sleep(10);
$upd_news_id=mysql_query("UPDATE wh_anonce_in_world SET id_news='$llasett' WHERE id='$llasett' ") or die ("Invalid query: " . mysql_error());
