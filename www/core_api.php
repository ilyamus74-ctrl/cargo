...
case 'add_new_item_in':
    ...
    if ($batchUid <= 0) {
        ...
        $batchUid     = (int)(microtime(true) * 1000000); // как uid_created
        $ownerUserId  = $operatorUserId;
    } else {
        // существующая партия — ищем её владельца
        $ownerUserId = $operatorUserId; // значение по умолчанию
    ...
    $rcCountryName = trim($_POST['receiver_country_name'] ?? '');
    ...
    $sizeL    = $_POST['size_l_cm'] ?? null;
    $sizeW    = $_POST['size_w_cm'] ?? null;
    $sizeH    = $_POST['size_h_cm'] ?? null;
    ...
    $deviceId   = null; // для веба; для мобилки можно сюда класть device_id
    ...
    $stmt->bind_param(
        ...
        $batchUid,
        $uidCreated,
        $ownerUserId, // В БД кладём ВЛАДЕЛЬЦА партии
        $deviceId,
        $tuid,
        $tracking,
        $carrierCode,
    ...
    audit_log(
        $operatorUserId,                 // в audit_logs.user_id — кто реально добавил
...
case 'commit_item_in_batch':
    ...
    // Админ или нет — для аудита
    $isAdmin = auth_has_role('ADMIN');

    // 1) Сколько ещё НЕзакоммиченных посылок в этой партии (без привязки к user_id!)
    $stmt = $dbcnx->prepare(
        "SELECT COUNT(*) AS cnt
           FROM warehouse_item_in
          WHERE batch_uid = ?
    ...
    if ($cnt === 0) {
        // Партия уже пустая или ранее была завершена — НЕ считаем это ошибкой
        $response = [
            'status'  => 'ok',
    ...
    // 2) Копируем все незавершённые строки партии в warehouse_item_stock
    ...
    // 3) Помечаем исходные записи как committed = 1
    $stmt = $dbcnx->prepare(
    ...
    // 4) Аудит — кто реально нажал "Готово"
    audit_log(
        $userId,                       // кто нажал
        'WAREHOUSE_IN_COMMIT',
        'WAREHOUSE_IN',
        $batchUid,                     // entity_id = UID партии
