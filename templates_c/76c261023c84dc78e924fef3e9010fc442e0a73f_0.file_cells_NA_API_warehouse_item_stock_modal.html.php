  ...
  <div class="col-12">
    <div class="d-flex justify-content-between align-items-start gap-2 mb-2">
      <div class="flex-grow-1">
        <div class="alert <?php if ($_smarty_tpl->getValue('item')['forwarder_registration_status'] == 'ok') {?>alert-success<?php } elseif ($_smarty_tpl->getValue('item')['forwarder_registration_status']) {?>alert-warning<?php } else { ?>alert-secondary<?php }?> mb-0">
          <div><strong>Регистрация у форварда:</strong> <?php echo htmlspecialchars((string)(($tmp = $_smarty_tpl->getValue('item')['forwarder_registration_status'] ?? null)===null||$tmp==='' ? '—' ?? null : $tmp), ENT_QUOTES, 'UTF-8', true);?>
</div>
          <div><strong>Дата регистрации:</strong> <?php echo htmlspecialchars((string)(($tmp = $_smarty_tpl->getValue('item')['forwarder_registered_at'] ?? null)===null||$tmp==='' ? '—' ?? null : $tmp), ENT_QUOTES, 'UTF-8', true);?>
</div>
          <?php if ($_smarty_tpl->getValue('item')['forwarder_registration_message']) {?>
            <div><strong>Сообщение:</strong> <?php echo htmlspecialchars((string)$_smarty_tpl->getValue('item')['forwarder_registration_message'], ENT_QUOTES, 'UTF-8', true);?>
</div>
          <?php }?>
        </div>
      </div>

      <?php if ($_smarty_tpl->getValue('can_forwarder_operations')) {?>
        <div class="dropdown">
          <button class="btn btn-outline-secondary"
                  type="button"
                  id="warehouseStockOpsMenu"
                  data-bs-toggle="dropdown"
                  aria-expanded="false"
                  title="Операции">
            ☰
          </button>
          <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="warehouseStockOpsMenu">
            <li>
              <button type="button"
...
"
    ...
    <div class="form-text">
      Меню операций использует текущие сохранённые данные карточки. Если изменяли поля — сначала нажмите “Сохранить”.
    </div>
  </div>
