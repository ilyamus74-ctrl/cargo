<?php
/* Smarty version 5.3.1, created on 2025-12-15 19:59:49
  from 'file:cells_NA_main.html' */

/* @var \Smarty\Template $_smarty_tpl */
if ($_smarty_tpl->getCompiled()->isFresh($_smarty_tpl, array (
  'version' => '5.3.1',
  'unifunc' => 'content_694068b508c5d4_00120897',
...
function content_694068b508c5d4_00120897 (\Smarty\Template $_smarty_tpl) {
          ...
          <div class="card">
            <div class="filter">
              ...
              <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow">
                ...
                <li><span class="dropdown-item active">Текущий месяц</span></li>
              </ul>
            </div>

            <div class="card-body pb-0">
              <h5 class="card-title">Активные пользователи <span>| Текущий месяц</span></h5>

              <p class="small text-muted mb-2">Активность считается по обработанным посылкам за месяц.</p>
              <p class="small mb-3">Количество активных пользователей: <strong><?php echo (($tmp = $_smarty_tpl->getValue('activeUsersCount') ?? null)===null||$tmp==='' ? 0 ?? null : $tmp);?>
</strong></p>

              <div id="activeUsersChart" style="min-height: 400px;" class="echart"></div>

              <?php echo '<script'; ?>
>
                document.addEventListener("DOMContentLoaded", () => {
                  const activity = <?php echo (($tmp = $_smarty_tpl->getValue('activeUsersMonthly') ?? null)===null||$tmp==='' ? '[]' ?? null : $tmp);?>
;

                  if (!activity.length) {
                    document.querySelector('#activeUsersChart').innerHTML = '<p class="text-center text-muted mb-0">Нет данных за текущий месяц</p>';
                    return;
                  }

                  const categories = activity.map((item) => item.user_name);
                  const processed = activity.map((item) => item.processed);

                  echarts.init(document.querySelector("#activeUsersChart")).setOption({
                    tooltip: {
                      trigger: 'axis',
                      axisPointer: {
                        type: 'shadow'
