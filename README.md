...
## Forwarder no-op audit diagnostics

Use these SQL snippets manually to inspect or clean up legacy no-op forwarder audit rows. Do not delete rows before taking a backup.

```sql
SELECT COUNT(*) AS noop_audit_count
FROM audit_logs
WHERE event_type = 'forwarder.sync.out.status_changed'
  AND JSON_VALID(extra_data)
  AND CAST(JSON_UNQUOTE(JSON_EXTRACT(extra_data, '$.affected_rows')) AS UNSIGNED) = 0;

CREATE TABLE audit_logs_forwarder_noop_backup AS
SELECT *
FROM audit_logs
WHERE event_type = 'forwarder.sync.out.status_changed'
  AND JSON_VALID(extra_data)
